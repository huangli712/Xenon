//! Facade entry points for SIMD element-wise dispatch (unary + binary).
//!
...
// ----------------------------------------------------------------------------
// Facade entry point — binary element-wise
// ----------------------------------------------------------------------------
    ...
    A: SimdElement,
{
    ...
    if tid == TypeId::of::<f32>() {
        ...
        return binary_simd::dispatch_binary_f32(op, lhs, rhs, dst);
    }
    if tid == TypeId::of::<f64>() {
        // SAFETY: TypeId check confirmed the concrete type; the unsized
        // coercion from &[A] to &[T] through raw pointers is sound
        ...
        return binary_simd::dispatch_binary_f64(op, lhs, rhs, dst);
    }
    if tid == TypeId::of::<Complex<f32>>() {
        // SAFETY: Complex<T> is repr(C) with two T fields; the layout is
        // identical to [T; 2]. The cast through raw pointers preserves
        ...
        return binary_simd::dispatch_binary_complex_f32(op, lhs, rhs, dst);
    }
    if tid == TypeId::of::<Complex<f64>>() {
        // SAFETY: Complex<T> is repr(C) with two T fields; the layout is
        // identical to [T; 2]. The cast through raw pointers preserves
        ...
        return binary_simd::dispatch_binary_complex_f64(op, lhs, rhs, dst);
    }
...
mod tests {
    ...
    /// Slices below the element-wise threshold must be rejected by binary
    /// dispatch, leaving `dst` untouched.
    #[test]
    fn test_binary_below_threshold_rejects() {
    ...
    /// Empty slices must be rejected by binary dispatch.
    #[test]
