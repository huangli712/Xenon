    ...
    match path {
        ...
        ExecPath::Parallel => {
            ...
            #[cfg(feature = "parallel")]
            {
                ...
                crate::parallel::reduce::par_dot::<_, _, A, _, _>(
                    a,
                    b,
                    &strategy,
                    guard,
...
mod tests {
    ...
    fn test_dot_rhs_high_rank_invalid_argument() {
        ...
        let b = Tensor::<i32, Ix2>::from_shape_vec((1, 1), vec![1_i32]).expect("valid construction");
        let err = dot(&a, &b).expect_err("must return error");
        match err {
            XenonError::InvalidArgument {
                kind: InvalidArgumentKind::OperationSpecific {
            ...
            other => panic!(
                "expected InvalidArgument::OperationSpecific for b, got {other:?}"
            ),
        }
    }
    ...
    fn test_dot_real_conjugate_is_identity() {
        let a = Tensor1::from_shape_vec(Ix1(3), vec![1.0_f64, -2.0, 3.0]).expect("valid construction");
        let b = Tensor1::from_shape_vec(Ix1(3), vec![4.0_f64, 5.0, -6.0]).expect("valid construction");
    ...
    fn test_dot_wire_dispatch_small_input_serial() {
        let a = Tensor1::from_shape_vec(Ix1(4), vec![1.0_f64, 2.0, 3.0, 4.0]).expect("valid construction");
        let b = Tensor1::from_shape_vec(Ix1(4), vec![5.0_f64, 6.0, 7.0, 8.0]).expect("valid construction");
        assert_eq!(
    ...
    fn test_dot_simd_path_with_feature() {
        ...
        let a = Tensor1::from_shape_vec(Ix1(values.len()), values.clone()).expect("valid construction");
        let b = Tensor1::from_shape_vec(Ix1(values.len()), values.clone()).expect("valid construction");
    ...
    fn f64_dot_tolerance_parallel(n: usize, max_abs_a: f64, max_abs_b: f64) -> f64 {
        ...
        let ulp_term =
            256.0 * f64::EPSILON * (n as f64) * max_abs_a * max_abs_b;
        let floor = 4.0 * f64::MIN_POSITIVE;
        ulp_term.max(floor)
    }
