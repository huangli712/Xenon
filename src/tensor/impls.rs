    ...
    D: crate::dimension::Dimension,
{
    ...
    pub fn layout_state(&self) -> crate::layout::LayoutState { self.flags.classify() }
...
impl<A> StorageSemantics for Owned<A> {
    ...
    fn alias_class(flags: crate::layout::LayoutFlags, derived: bool) -> AliasClass {
        if flags.has_zero_stride() { AliasClass::BroadcastAlias }
        else if derived { AliasClass::ViewMutDerived }
        else { AliasClass::Unique }
    }
}
    ...
    D: crate::dimension::Dimension,
{
    ...
    pub fn storage_kind(&self) -> StorageKind { S::KIND }
    ...
    D: crate::dimension::Dimension,
{
    ...
    pub fn storage_len(&self) -> usize { self.storage.len() }
    ...
    D: crate::dimension::Dimension,
{
    ...
    pub fn as_storage_mut_ptr(&mut self) -> *mut A { self.storage.as_mut_ptr() }
    ...
    D: crate::dimension::Dimension + Clone,
{
    ...
    pub fn view(&self) -> TensorBase<ViewRepr<'_, A>, D> {
        ...
        let storage = unsafe { ViewRepr::from_raw_parts(self.storage.as_ptr(), self.storage.len()) };
        unsafe {
            TensorBase::new_unchecked(
                storage, self.shape.clone(), self.strides.clone(),
                self.offset, self.flags, false,
            )
        }
    }
}
    ...
    D: crate::dimension::Dimension + Clone,
{
    ...
    pub fn view(&self) -> TensorBase<ViewRepr<'_, A>, D> {
        ...
        unsafe {
            TensorBase::new_unchecked(
                storage, self.shape.clone(), self.strides.clone(),
                self.offset, self.flags, self.derived_from_view_mut,
            )
        }
    }
}
...
mod tests {
    ...
    fn make_owned(shape: Ix2, data: Vec<f64>, flags: LayoutFlags, derived: bool,
                  offset: usize) -> TensorBase<Owned<f64>, Ix2>
    {
        let strides = compute_f_strides(&shape).expect("valid shape");
        let storage = Owned::from_vec(data).expect("valid vec");
        TensorBase { storage, shape, strides, offset, flags, derived_from_view_mut: derived }
    }

    fn f_contig(data: Vec<i32>, shape: Ix2, offset: usize) -> TensorBase<Owned<i32>, Ix2> {
        let strides = compute_f_strides(&shape).expect("valid shape");
        let storage = Owned::from_vec(data).expect("valid vec");
        TensorBase { storage, shape, strides, offset, flags: LayoutFlags::F_CONTIGUOUS,
    ...
    #[test] fn test_tensor_shape_strides() {
        let t = f_contig_i32(vec![1; 12], Ix2(3, 4));
        assert_eq!(t.shape(), &[3, 4]);
        assert_eq!(t.strides(), &[1_usize, 3]);
    }
    #[test] fn test_tensor_len_and_ndim() {
        let t = f_contig_i32(vec![1; 12], Ix2(3, 4));
        assert_eq!(t.len(), 12); assert_eq!(t.ndim(), 2);
    }
    #[test] fn test_tensor_offset_and_flags() {
        let strides = compute_f_strides(&Ix1(5)).expect("valid");
        let storage = Owned::from_vec(vec![1_i32; 5]).expect("valid");
        let t = TensorBase::<Owned<i32>, Ix1> { storage, shape: Ix1(5), strides,
            offset: 0, flags: LayoutFlags::F_CONTIGUOUS, derived_from_view_mut: false };
        assert_eq!(t.offset(), 0);
        assert!(t.flags().is_f_contiguous());
    }
    #[test] fn test_tensor_raw_dim() {
        let t = f_contig_i32(vec![1; 4], Ix2(2, 2));
        assert_eq!(t.raw_dim().slice(), &[2, 2]);
    }
    #[test] fn test_tensor_is_empty() {
        let t = f_contig_i32(Vec::<i32>::new(), Ix2(0, 3));
        assert!(t.is_empty());
    }
    #[test] fn test_tensor_data_location() {
        let t = f_contig_i32(vec![1], Ix2(1, 1));
        assert_eq!(t.data_location(), DataLocation::Cpu);
    }
    #[test] fn test_tensor_storage_kind_owned() {
        let t = f_contig_i32(vec![1; 4], Ix2(2, 2));
        assert_eq!(t.storage_kind(), StorageKind::Owned);
    }
    #[test] fn test_tensor_access_semantics_owned() {
        let t = f_contig_i32(vec![1; 4], Ix2(2, 2));
    ...
    #[test] fn test_layout_state_f_contiguous() {
    ...
    #[test] fn test_layout_state_non_contiguous() {
        let t = make_owned(Ix2(2,3), vec![0.0; 6], LayoutFlags::EMPTY, false, 0);
        assert!(!t.is_f_contiguous());
        assert_eq!(t.layout_state(), LayoutState::NonContiguous);
    }
    #[test] fn test_flags_aligned() {
        let t = make_owned(Ix2(2,3), vec![0.0; 6],
                           LayoutFlags::F_CONTIGUOUS.set_aligned(true), false, 0);
        assert!(t.is_aligned());
    }
    #[test] fn test_not_aligned() {
        let t = make_owned(Ix2(2,3), vec![0.0; 6], LayoutFlags::EMPTY, false, 0);
        assert!(!t.is_aligned());
    }
    #[test] fn test_has_zero_stride_set() {
        let t = make_owned(Ix2(2,3), vec![0.0; 6], LayoutFlags::HAS_ZERO_STRIDE, false, 0);
        assert!(t.has_zero_stride());
    }
    #[test] fn test_has_zero_stride_clear() {
        let t = make_owned(Ix2(2,3), vec![0.0; 6], LayoutFlags::F_CONTIGUOUS, false, 0);
        assert!(!t.has_zero_stride());
    }
    #[test] fn test_alias_class_unique_owned() {
        let t = make_owned(Ix2(2,3), vec![0.0; 6], LayoutFlags::F_CONTIGUOUS, false, 0);
        assert_eq!(t.alias_class(), AliasClass::Unique);
    }
    #[test] fn test_alias_class_broadcast() {
        let t = make_owned(Ix2(2,3), vec![0.0; 6], LayoutFlags::HAS_ZERO_STRIDE, false, 0);
        assert_eq!(t.alias_class(), AliasClass::BroadcastAlias);
    }
    #[test] fn test_alias_class_view_mut_derived() {
    ...
    #[test] fn test_as_slice_non_contiguous() {
        ...
        let t: TensorBase<Owned<i32>, Ix2> = TensorBase {
            storage, shape, strides, offset: 0, flags: LayoutFlags::EMPTY,
            derived_from_view_mut: false };
    ...
    #[test] fn test_view_mut_writable() {
    ...
    #[test] fn test_view_from_view_mut_derived_flag() {
        let mut t = f_contig_i32(vec![1,2,3,4], Ix2(2,2));
        let vm = t.view_mut();
        let downgraded = vm.view();
        assert!(downgraded.derived_from_view_mut);
        assert_eq!(downgraded.access_semantics(), AccessSemantics::SharedReadOnly);
    }
    #[test] fn test_view_mut_reborrow_no_flag() {
        let mut t = f_contig_i32(vec![1,2,3,4], Ix2(2,2));
    ...
    #[test] fn test_view_from_owned_read_only() {
        let t = f_contig_i32(vec![1,2,3,4], Ix2(2,2));
        let v = t.view();
