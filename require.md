...
> **注**：虽然 Complex 不实现 `Ord`/`PartialOrd`，但 `unique`/`unique_counts`/`unique_inverse` 等排序相关操作使用实部/虚部字典序的 `total_cmp`（见 §11.3 unique 语义表）。这与 `Ord` trait 无关——排序函数内部使用自定义比较闭包，不依赖 `Ord` 实现。
