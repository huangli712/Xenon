...
| ALIGNED | bit 2 | SIMD 对齐（64 字节） | 缓存对齐状态，用于**快速排除**不可能对齐的路径（ALIGNED=false 时直接跳过对齐加载尝试）。**不得**作为选择加载指令的唯一依据——运行时仍须检查 `ptr as usize % 64 == 0`（小数组降级后 ALIGNED=false 但地址可能恰好对齐，见 §9.1.4）。ALIGNED=true 时可跳过运行时地址检查作为优化 |
...
| 嵌套并行 | 禁止嵌套并行（内层操作强制单线程），避免线程池饥饿。**强制机制**：使用 `thread_local!` + RAII scope guard 标记并行上下文，并行操作入口处检查标记——若已在并行上下文中，内层操作强制走单线程路径（不 spawn 新任务）。**panic 安全**：须使用 RAII guard（Drop 时自动重置 flag），防止 panic 后 flag 残留 true 导致 rayon 线程复用时后续操作静默退化为单线程。实现示例：`thread_local! { static IN_PARALLEL: Cell<bool> = Cell::new(false); }` + `struct ParallelGuard; impl Drop for ParallelGuard { fn drop(&mut self) { IN_PARALLEL.with(|c| c.set(false)); } }`，并行入口：`IN_PARALLEL.with(\|c\| c.set(true)); let _guard = ParallelGuard; /* guard 在作用域结束时（含 panic unwind）自动重置 */`，内层检查 `if IN_PARALLEL.with(\|c\| c.get()) { return scalar_path(); }` |
...
|--------|------|----------|
| 1 | 元素数 < SIMD 宽度 | 标量路径 |
| 2 | 元素数 ≥ SIMD 宽度 且 宽松连续内存（`is_padded_contiguous()`）且 **所有步长为正值或零（size-1 轴允许 0，见 §9.1.3）** 且 simd 启用 且 操作有 SIMD 实现（见注） | SIMD 路径（单线程） |
| 3 | 元素数 ≥ 并行阈值 且 parallel 启用 且 宽松连续内存（`is_padded_contiguous()`）且 **所有步长为正值或零（size-1 轴允许 0）** 且 simd 启用 且 操作有 SIMD 实现 | SIMD + 并行路径（每线程内部使用 SIMD） |
...
| 6 | 元素数 ≥ 并行阈值 且 parallel 启用 且 宽松连续内存 但 **存在负步长**（满足 padded contiguous 但步长方向不对；注：步长=0 不算负步长，见 §9.1.3） | 标量 + 并行路径（SIMD 条件不满足，回退标量但可并行） |
| 7 | 宽松连续内存 且 **所有步长为正值或零（size-1 轴允许 0）** 且 simd 未启用 且 元素数 < 并行阈值（或 parallel 未启用） | 标量路径（连续内存但 SIMD 未启用，单线程标量迭代） |
| 8 | 非宽松连续内存 或 **存在负步长** 且 元素数 < 并行阈值（或 parallel 未启用） | 标量路径（单线程，步长跳跃遍历） |

**条件依赖关系图**：

```
is_padded_contiguous() && all_strides_positive_or_zero() && simd_enabled && has_simd_impl
...
上述 8 级分层表描述的是单操作数（一元/归约）的 dispatch 逻辑。二元操作（如 `a + b`）须在进入分层表之前执行额外的前置检查，通过后再按输出数组的属性走分层表。具体流程：
...
binary_dispatch(a, b, op):
    ...
    // 阶段 3：存储复用判定
    if can_reuse_output_storage(a, b):
        → 选择 a 或 b 的存储作为输出，避免分配（见 §19.x 输出存储策略）

    // 阶段 4：按输出属性走 8 级分层表
    // 输出数组的 contiguous/stride/size 属性决定最终执行路径
    → fallback to tier_table(output_array)
```
...
| 3. 存储复用 | 输出可复用某操作数存储（所有权/对齐/容量满足） | 就地写入 | §19.x |
| 4. 分层回退 | 按输出数组属性匹配 8 级分层表 | 对应层级路径 | §9.3 主表 |
...
8 级分层表 + 二元 dispatch 规则覆盖 30+ 操作，每个操作手动实现分支会导致大量重复代码。推荐以下实现策略：
...
- `iter_padded_contiguous()` → `Option<PaddedContiguousIter>`：宽松连续数组（`is_padded_contiguous()` = true，含严格连续）可用，**实现 `DoubleEndedIterator`**（内部跳过 padding 字节，反向遍历通过维护行/列起止指针实现）
...
| 填充数组 | 返回填充后的物理步长（如 F-order M_padded）。**示例**：一个逻辑 4×4 矩阵按 F-order 填充到 M_padded=16，则 `strides=[1, 16]`，`lda()` 返回 `16`（即 `|strides[1]|`）。可直接传递给 BLAS 的 LDA 参数 | 可直接传递给 BLAS 的 LDA 参数 |
