...
| unique_counts | 返回唯一值及出现次数 | 可排序类型（整数、浮点、复数） | (Tensor1<A>, Tensor1<usize>) | 任意维度（先按 F-order 展平为一维） |
| unique_inverse | 返回唯一值及原数组索引 | 可排序类型（整数、浮点、复数） | (Tensor1<A>, Tensor1<usize>) | 任意维度（先按 F-order 展平为一维） |
...
**排序能力需求**：集合操作中的 unique/unique_counts/unique_inverse 需要对元素进行排序和去重。由于浮点类型（NaN 为偏序）和复数类型（无自然全序）不实现 `Ord`，需要一个独立的排序能力 trait（sealed），覆盖整数（自然全序）、浮点（IEEE 754 totalOrder）、复数（字典序：先实部后虚部，使用 totalOrder）。bool 不需要（unique 无实际意义），usize 仅作为索引返回类型不参与排序。此 trait 应为 `Element` 的子 trait，与 `Numeric`/`RealScalar`/`ComplexScalar` 并列。
...
| unique_inverse 索引语义 | inverse 中的索引为展平后的一维索引（0-based），非原始多维坐标 |
| bincount 不支持多维 | bincount 的语义与多维结构无关，仅接受一维输入 |
...
| inverse 索引语义 | inverse 中的索引对应展平后的一维序列，即 inverse[i] 表示展平后第 i 个元素在 values 中的位置 |

### 15.5 bincount 语义

| 属性 | 行为 |
|------|------|
| 输入约束 | 仅支持 i32 / i64 / usize。i32/i64 的值须 ≥ 0（运行时检查），usize 天然非负。**不支持** bool 和浮点类型 |
| minlength 参数 | 指定输出最小长度。默认值为 0（输出长度由输入最大值决定） |
| 空数组 | 返回长度为 minlength（默认 0）的全零数组 |
| 全零输入 | 输入全为 0 时，输出长度为 max(1, minlength)，output[0] = input.len() |
| 负值输入 | i32/i64 输入含负值时 panic |
| 输出长度溢出保护 | 输出长度为 max(input) + 1，若超过 isize::MAX，返回 InvalidInput 错误（而非 panic 或 OOM） |
| 返回值 | Tensor1<usize>，长度为 max(input) + 1（或 minlength，取较大值），output[i] = count of i in input |
| 权重参数 | 可选 weights: Tensor1\<W\>（W: Numeric），权重元素类型独立于输入整数类型。带权重时返回 Tensor1\<W\>，output[i] = sum of weights[j] where input[j] == i。weights 长度须与 input 长度相等，否则 panic |

### 15.6 histogram 语义

| 属性 | 行为 |
|------|------|
| bins 参数 | 接受等宽 bin 数（usize）或自定义 bin 边界数组（Tensor1<A>） |
| range 参数 | (min, max) 元组，指定统计范围，超出范围的值不计入。bins 为自定义边界数组时不允许同时提供 range，否则返回 InvalidInput 错误 |
| 空数组 | 返回全零数组（长度等于 bin 数） |
| 返回值 | Tensor1<usize>，长度等于 bin 数 |
| NaN 输入 | NaN 不计入任何 bin。range 已指定时静默跳过 NaN；range 未指定（需从数据推断范围）且输入含 NaN 时，返回 InvalidInput 错误 |
| bin 边界规则 | 第 i 个 bin（0 ≤ i < k-1）的区间为 [ei, ei+1)（左闭右开），最后一个 bin 的区间为 [ek-1, ek]（左闭右闭）。与 NumPy 行为一致 |
| 自定义边界数组验证 | 须满足：长度 ≥ 2、严格单调递增、不含 NaN、不含 Inf。违反则返回 InvalidInput 错误 |
...
| 属性 | 行为 |
|------|------|
| bins 参数 | 等宽 bin 数或自定义边界 Tensor1<A>（直接返回 clone） |
| range 参数 | (min, max) 元组，指定等宽分割的上下界。bins 为自定义边界数组时不可提供 |
| 等宽边界计算 | 给定 bin 数 k 和 range (lo, hi)，均匀分割为 k+1 个边界 |
| 默认 range | bins 为整数且未提供 range 时，从输入数据推断（min/max）。输入含 NaN 时返回 InvalidInput |
| 返回值 | Tensor1<A>，长度等于 bin 数 + 1 |
| 空输入 | bins 为整数且输入为空数组时，返回 InvalidInput（无法从空数据推断 range） |
...
| 轴操作（sum_axis / slice / index_axis 等） | `InvalidAxis` | axis >= ndim 时返回 `InvalidAxis`（Result）。注意：s![] 宏中轴数不匹配的行为不同——静态维度编译错误，动态维度 panic（见 §13.9） |
...
| 策略 | 适用场景 | 典型例子 |
|------|----------|----------|
| 可恢复错误（形状、布局、轴、数据驱动索引越界） | 返回 Result | squeeze 非 size-1 轴 → `InvalidShape`；take/put 索引数组越界 → `IndexOutOfBounds` |
