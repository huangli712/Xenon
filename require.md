...
- **IxDyn `Vec<usize>` 分配优化**：`IndexedIter` 内部使用 `InlineArray<usize, 8>` 作为坐标计数器（与广播迭代器一致，见 §11.2），每次 `next()` 将内部坐标 clone 到 `Vec<usize>` 输出。对于 ≤ 8 维的常见情况，内部计数器在栈上分配，每次 `next()` 仅产生一次 `Vec::from_iter()` 的堆分配（从栈上固定大小数组复制）。**若需完全消除堆分配**，可使用 `iter()` 配合手动坐标计算（或使用 `enumerate()` 获取展平索引），或提供 `indexed_iter_no_alloc()` 方法返回内部坐标的引用（牺牲 `Iterator` trait 兼容性）。后续版本可考虑将 IxDyn 的 `D::Pattern` 从 `Vec<usize>` 改为 `SmallVec<[usize; 8]>` 以从根源消除分配
