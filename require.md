...
| 方法 | 签名 | 说明 |
|------|------|------|
| strides_bytes() | `&self -> Vec<isize>` | 返回各轴步长的向量（字节单位，有符号），每次调用重新计算 `O(ndim)`，逐元素计算 `stride_in_bytes = (stride as isize) * size_of::<A>()`。与 `strides()` 的 `&[isize]`（零拷贝 transmute）不同，字节单位步长无法通过 transmute 获得（需要逐元素乘以 `size_of::<A>()`），因此返回新分配的 `Vec<isize>`。`O(ndim)` 的分配和计算在 FFI 场景下可接受（ndim 通常 ≤ 6）。若需频繁调用，建议调用方缓存结果 |
