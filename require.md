...
| `type Reduced` | 关联类型 | 沿一轴归约后的维度类型（Ix1 → Ix0, Ix2 → Ix1, ..., IxDyn → IxDyn）。**限制**：`Reduced` 仅编码"维度数减 1"的类型映射，不编码具体移除的是哪个轴——例如 `Ix3` 无论移除 axis(0) 还是 axis(2)，`Reduced` 类型均为 `Ix2`。具体轴选择由 `remove_axis()` 运行时参数决定，类型系统无法在编译时区分。这是 Rust 关联类型无法表达类型级轴信息的已知限制（与 ndarray 设计一致） |
...
> **设计决策**：`zero()`/`one()` 由本库在 `Element` trait 中自行定义，而非依赖任何外部 crate（如 `num-traits`）。原因：本库的 `Element` 是 sealed trait，实现者集合完全由库控制（仅标准数值类型），无需 `num-traits` 的泛型抽象能力；自行定义可避免外部依赖版本冲突、减少编译依赖、保证 `zero()`/`one()` 语义与库内其他 trait（`Numeric`、`ComplexElement`）一致，且允许未来针对 SIMD 优化返回值（如确保对齐的零向量）。
...
Storage trait 预留 `type Device` 关联类型，当前版本仅支持 `Cpu`。GPU 支持计划在下一版本实现。

**设计约束（当前版本须满足）**：

1. **存储层与设备解耦**：`TensorBase<S, D>` 的 `S` 参数承载设备信息（通过 `S::Device` 关联类型），而非在 `TensorBase` 层面增加独立泛型参数。这保证 `TensorBase<S, D>` 签名不变即可支持新设备。
2. **迭代器与设备无关的接口分离**：当前所有迭代器（`iter()` / `iter_mut()` 等）隐式假设数据在 Host 内存可解引用。GPU 存储不可在 Host 端直接解引用，因此 GPU 后端须提供独立的执行模型（如 kernel launch），不可复用现有迭代器 API。
3. **Shape / Stride / Layout 可复用**：形状、步长、布局标志为纯元数据，与存储位置无关。GPU 后端可直接复用 `Shape<D>` 和 `LayoutFlags`，无需重新定义。

**已调研的设计参考**（下一版本决策依据）：

| 决策项 | 候选方案 | 参考项目 |
|--------|----------|----------|
| 后端抽象 | 关联类型 + trait 组合（如 `trait TensorBackend { type Device; type Tensor<T, D>; }`） | burn `Backend` trait |
| Host↔Device 传输 | 显式 `.to(device)` 方法，不隐式零拷贝 | tch-rs `.to(device)`、burn `staging()` |
| 元素类型约束 | GPU 安全 trait（要求 `Copy` + 可位拷贝，不含引用/指针） | cust `DeviceCopy` trait |
| 存储模型 | 独立 `Storage` enum（`CpuStorage` / `CudaStorage`），不依赖 ndarray 的 `Data` trait | candle `Storage` enum |
...
> `shape()` / `strides()` / `offset()` 的完整语义在此处定义。FFI 集成场景下的补充方法（如 `strides_bytes()`）见 §14.2。
...
> **设计决策**：所有 SIMD 路径统一使用 pulp crate 抽象，包括 SSE4.1。若 pulp 默认 feature 配置未覆盖 SSE4.1 分派级别（x86_64 默认仅 V3→Scalar），需通过 pulp 的 feature gate（如 `x86-v2`）显式启用 SSE4.1 分派路径，确保无 AVX2 的 x86_64 环境（旧服务器、嵌入式、云实例）仍可获得 128-bit SIMD 加速，而非直接回退标量。
...
**mean/var/std 类型约束**：`mean()`、`var()`、`std()` 仅对浮点类型（`f32`/`f64`）实现。整数数组调用这些方法将产生编译错误。实现机制：这些方法的 `where A: RealScalar` 约束（见 §2.5）在编译时排除了整数类型（整数仅实现 `Numeric`，不实现 `RealScalar`）。用户需先手动转换类型，例如 `.mapv(|x| x as f64).mean()`。理由：Xenon 不做隐式类型提升，整数除法截断会产生误导性结果。
...
> **性能提示**：`reshape()` 对非连续输入会隐式执行完整数据拷贝。性能敏感场景下，调用方可通过 `is_contiguous()` 预检：若返回 `true`，后续 `reshape()` 保证零拷贝；若返回 `false`，调用方可决定是否先调用 `as_f_contiguous()` / `as_c_contiguous()`（见 §13.4）获取零拷贝视图再操作，或接受拷贝开销。
...
| 返回值 | 维度类型与 self 相同（`D`），指定轴长度为 mask 中 true 的数量（运行时决定，静态维度 `D` 的 shape 本身就是运行时值，因此无矛盾）。其余轴长度不变 |
...
| 内存布局 | 默认 F-contiguous（列优先，与 §5.1 项目默认布局一致） |

**from_fn 闭包调用顺序**：闭包按 F-order（列优先，最右轴变化最快）遍历所有索引。闭包签名 `Fn(D::Pattern) -> A`，参数为多维索引。调用顺序保证确定性——相同输入始终产出相同数组。闭包内不应依赖调用时序做副作用。**输出布局**：from_fn 构造的数组始终为 F-contiguous（与 §5.1 项目默认布局一致）。
...
> 以上方法支持广播（与 §10.4 广播规则一致），始终分配新数组。`PartialEq` 的 `==`/`!=` 运算符保持标量语义（`Tensor<A, D> == Tensor<A, D> -> bool`，判断两个数组形状和所有元素是否完全相等）。**NaN 行为**：由于 `PartialEq` 遵循 IEEE 754（`NaN != NaN`），两个形状相同且在相同位置均为 NaN 的 Tensor，`==` 比较结果为 `false`（因为逐元素比较时 NaN != NaN）。若需 NaN 敏感的相等判断，使用 `allclose(rtol=0.0, atol=0.0)`（但注意 `allclose` 同样将 NaN 比较为 false，见 §13.3）。
...
| map/mapv/mapv_into/mapv_inplace | 映射操作（语义见 §13.3 map 语义） |
| cast | 显式类型转换（语义见 §13.5 cast 语义） |
...
**泛型约束**：`B: Element` 约束保证目标类型在本库的类型体系内（Sealed trait，见 §2.2）。转换规则为逐元素按值转换，始终成功（溢出使用饱和语义，见下方精度表）。不支持跨类别隐式转换（如 `Complex → 实数` 须显式取 `re()`）。
...
| into_raw_parts | `fn into_raw_parts(self) -> (*const A, D, D::Stride, usize)` | 消耗数组，返回 (ptr, shape, strides, offset) 元组。**内存释放责任**：调用方获得原始指针后须自行管理内存生命周期——通过 `from_raw_parts` 重新构造为 Tensor 交由 Drop 释放，或使用全局分配器的 `dealloc` 手动释放（须与分配时 `Layout` 一致）。忘记释放将导致内存泄漏 |
...
| `borrow_aligned` | `fn borrow_aligned<A>(&mut self, len: usize) -> &mut [A]` | 借出指定元素数的类型化缓冲区，按 `max(align_of::<A>(), 当前对齐)` 对齐。对齐不足时 panic |
...
| cat / stack | `InvalidShape`（0 个输入、ndim 不一致、非拼接轴长度不等）/ `ShapeMismatch`（非拼接轴长度不等时也可返回此错误以区分形状问题的性质） | 独立错误 |
| squeeze / broadcast | `InvalidShape` | 独立错误 |
| 轴操作（sum_axis / cumsum / slice / index_axis 等） | `InvalidAxis` | axis >= ndim 时返回 `InvalidAxis`（Result）。注意：s![] 宏中轴数不匹配的行为不同——静态维度编译错误，动态维度 panic（见 §12.2） |
...
| `iter_mut()` 对广播视图 panic（见 §9.3） | 未修改任何元素；Tensor 保持原状态 | panic 在写入前触发，数据完好 |
...
| 容差 | 不超过 §17.4 精度表中规定的阈值；超越函数的 ULP 阈值：f64 ≤ 4 ULP，f32 ≤ 4 ULP |
