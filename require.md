...
| 逐元素二元（浮点/复数） | add, sub, mul, div | 两操作数均连续内存（步长=1）且形状兼容；任一操作数步长≠1 回退标量路径 |
| 逐元素二元（整数） | add, sub, mul | 两操作数均连续内存（步长=1）且形状兼容；任一操作数步长≠1 回退标量路径 |
...
|------|----------|
| 元素数 < SIMD 宽度 | 标量路径 |
| 元素数 ≥ SIMD 宽度 且 连续内存 且 simd 启用 且 操作有 SIMD 实现（见注） | SIMD 路径 |
| 元素数 ≥ 并行阈值 且 parallel 启用 且 连续内存 且 simd 启用 且 操作有 SIMD 实现 | SIMD + 并行路径（每线程内部使用 SIMD） |
