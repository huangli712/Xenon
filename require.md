...
> **注**：虽然 Complex 不实现 `Ord`/`PartialOrd`，但 `unique`/`unique_counts`/`unique_inverse` 等排序相关操作使用实部/虚部字典序的 `total_cmp`（见 §11.2 unique 语义表）。这与 `Ord` trait 无关——排序函数内部使用自定义比较闭包，不依赖 `Ord` 实现。
...
> `shape()` / `strides()` / `offset()` 的完整语义在此处定义。FFI 集成场景下的补充方法（如 `strides_bytes()`）见 §15.2。
...
> `ShapeError` 将形状操作可能遇到的两种失败（形状不匹配 / 布局不满足）聚合为一个枚举，简化调用方错误处理。reshape（§12.2）不再使用 `ShapeError`（布局不匹配时自动拷贝，仅形状不匹配返回 `InvalidShape`）。`ShapeError` 保留供其他可能需要同时报告这两种错误的 API 使用。
...
| 轴操作（sum_axis / cumsum / slice / index_axis 等） | `InvalidAxis` | axis >= ndim 时返回 `InvalidAxis`（Result）。注意：s![] 宏中轴数不匹配的行为不同——静态维度编译错误，动态维度 panic（见 §13.2） |
