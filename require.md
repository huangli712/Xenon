...
| 归约（整数 sum） | sum | **默认使用 widening 累加 + SIMD 路径**。具体策略：`i8`/`u8` 累加到 `i32`/`u32`，`i16`/`u16` 累加到 `i64`/`u64`，`i32`/`u32` 累加到 `i64`/`u64`，`i64`/`u64` 因无更大标准整数类型，回退标量 checked 路径。Widening 累加器通过 SIMD widen 指令（如 `_mm256_cvtepi8_epi32`）实现元素扩展后累加，无分支、无溢出检测开销。累加完成后 widening 结果转为目标类型时检查是否超出范围，超出则 panic（极端罕见，需累加器本身溢出才会触发）。**opt-in checked 变体**：`sum_checked()` 方法保留逐元素 checked 算术（标量路径），用于需要严格溢出检测的场景。参考：rayon 的 `ParallelIterator::sum()` 使用 `wrapping_add`；numpy 的整数 sum 不检测溢出 |
| 归约（整数 prod） | prod | **默认使用 widening 累加 + SIMD 路径**（同 sum 策略）。Widening 累加器通过 SIMD widen 指令扩展后逐元素乘法累加。乘法溢出风险高于加法（累加值增长更快），widening 范围内的溢出不会发生，但 widening 结果转目标类型时仍须检查。`prod_checked()` 为 opt-in checked 变体（标量路径） |
...
| i64/u64 整数 sum/prod（默认 widening 路径） | i64/u64 无更大标准整数类型作为 widening 累加器，默认回退标量 checked 路径。可 opt-in 使用 `wrapping` 语义走 SIMD 路径（参见 §9.1.3 归约表格） |
...
binary_dispatch(a, b, op):
    ...
    // 阶段 2：广播 SIMD 快速路径
    if a.is_padded_contiguous() && b.has_zero_stride():
        → 广播 SIMD 路径（broadcast + SIMD，见 §16.7）
    if b.is_padded_contiguous() && a.has_zero_stride():
        → 广播 SIMD 路径（broadcast + SIMD，见 §16.7）
...
| 2. 广播 SIMD | 一方 padded contiguous 且另一方 zero-stride | 广播 SIMD 路径 | §16.7 |
| 3. 存储复用 | 输出可复用某操作数存储（所有权/对齐/容量满足） | 就地写入 | 本节（见上方阶段 3 注释） |
| 4. 分层回退 | 按输出数组属性匹配 5 级分层表 | 对应层级路径 | §9.3 主表 |

**注**：阶段 2（广播 SIMD）要求广播操作数的步长为 0 且被广播维度 size 为 1，连续操作数须满足 `is_padded_contiguous()` 且 `all_strides_positive_or_zero()`。不满足时跳过阶段 2，直接进入阶段 4。
...
反向遍历 padded 数组须正确跳过行/列尾 padding，比"维护起止指针"复杂。以下是 C-order（行优先）2D padded 数组的 `next_back()` 算法，更高维度通过递归/循环嵌套同理：
...
next_back():
    ...
    if back_ptr < row_start:
        // 跨越到上一行：须跳过上一行尾部 padding + 当前行整行
        // 计算上一行起始：row_start - padded_stride[last_axis] * size_of::<A>()
