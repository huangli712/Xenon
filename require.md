...
采用 9 标志位设计（2 字节 u16，低 9 位有效，高 7 位保留，须为 0），缓存高频查询的派生状态：
...
| is_c_padded_contiguous() | 是否 C-order 宽松连续（允许主维度填充）。判定规则：ndim≤1 时等价 `is_c_contiguous()`；ndim≥2 时要求 |strides[ndim-1]|==1、|strides[ndim-2]|≥shape[ndim-1]、|strides[i]|==|strides[i+1]|×max(shape[i+1], 1)（i≤ndim-3）。**注意**：当 shape[i+1]==1 时，条件退化为 |strides[i]|==|strides[i+1]|，防止 size-1 维度引入非连续跳跃 | O(1) |
...
| flags: u16 | 布局标志位（低 9 位有效，高 7 位保留为 0）。缓存高频查询的派生状态（F/C 连续性、对齐、零步长、负步长、填充），使布局查询方法达到 O(1) 复杂度（见 §7.3 标志定义与 §7.4 查询方法）。创建时根据分配方式初始化，切片/转置/reshape 时重新计算，视图创建时继承并按需降级 |
...
| `is_c_padded_contiguous()` | `&self -> bool` | 是否 C-order 宽松连续（允许填充）。判定规则详见 §7.4，size-1 内部维度步长须与后一轴步长一致 |
...
| 归约（整数 sum） | sum | **默认使用 widening 累加 + SIMD 路径**。具体策略：`i8`/`u8` 累加到 `i32`/`u32`，`i16`/`u16` 累加到 `i64`/`u64`，`i32`/`u32` 累加到 `i64`/`u64`，`i64`/`u64` 因无更大标准整数类型，**默认使用 wrapping SIMD 路径**（与 rayon `ParallelIterator::sum()` 和 NumPy 整数 sum 行为一致——均不检测溢出）。Wrapping SIMD 累加无分支、无溢出检测开销，性能与 `i32`/`u32` 的 widening 路径相当。**opt-in checked 变体**：`sum_checked()` 方法保留逐元素 checked 算术（标量路径），用于需要严格溢出检测的场景（如金融计算、安全敏感代码）。参考：rayon 的 `ParallelIterator::sum()` 使用 `wrapping_add`；numpy 的整数 sum 不检测溢出 |
| 归约（整数 prod） | prod | **默认使用 widening 累加 + SIMD 路径**（同 sum 策略）。Widening 累加器通过 SIMD widen 指令扩展后逐元素乘法累加。乘法溢出风险高于加法（累加值增长更快），widening 范围内的溢出不会发生，但 widening 结果转目标类型时仍须检查。`i64`/`u64` 默认使用 wrapping SIMD 路径（同 sum 策略）。`prod_checked()` 为 opt-in checked 变体（标量路径） |
...
| 参考标准 | 下列操作 `has_simd_impl = false`（见下方 "无 SIMD 实现" 表格）：整数除法、u8/i8 乘法、超越函数。其余操作 `has_simd_impl = true`（包括 i64/u64 wrapping SIMD 的 sum/prod） |
...
**注 — i64/u64 整数 sum/prod**：i64/u64 无更大标准整数类型作为 widening 累加器，默认使用 **wrapping SIMD 路径**（无溢出检测，与 rayon `wrapping_add`/NumPy 行为一致，`has_simd_impl = true`）。`sum_checked()`/`prod_checked()` 为 opt-in checked 变体（标量路径，逐元素 checked 算术，用于金融计算等溢出敏感场景）
...
| 检查阶段 | 条件 | 执行路径 | 对应章节 |
|----------|------|----------|----------|
| 1. 布局兼容性 | 两操作数布局方向不一致（C-padded 混合 F-padded） | 标量路径 | §9.1.3 |
...
| 3 | 以上均不满足 | 新分配 Owned 存储，默认 64 字节对齐，PADDED = false，布局方向继承操作数的共同方向（均为 C-padded → C-order，均为 F-padded → F-order，混合时默认 C-order） | 新分配保证容量和对齐 |
...
| zip 形状不一致但可广播 | 自动广播后迭代，等价于先 broadcast 再 zip。广播视图使用零步长实现逻辑重复（无额外堆分配），步长为 0 的轴在迭代时重复产出同一位置的数据。**遍历顺序规则**：若未指定遍历顺序，使用库默认 C-order（广播视图本身无物理内存，"物理布局顺序"在此场景下无意义，统一使用 C-order 保证行为确定性）。若显式指定遍历顺序（F 或 C），则按指定逻辑顺序迭代，可能产生非连续访问。**性能提示**：当所有非广播操作数均为 F-contiguous 且均为大数组时，C-order 遍历会对 F-contiguous 数组产生完全非连续访问（cache miss 严重）。用户应显式指定 F-order 遍历（`zip!(...).with_order(Order::F)`）以获得最优性能。未来版本可考虑在所有非广播操作数一致为 F-contiguous 时自动选择 F-order 作为广播遍历方向 |
...
| `iter_padded_contiguous()` | `fn iter_padded_contiguous(&self) -> Option<PaddedContiguousIter<'_, A, D>>` | `&A` | 所有存储模式 | 宽松连续数组（`is_padded_contiguous()` = true）返回 `Some`，包括严格连续数组和含 padding 的填充连续数组。迭代器在每行/列末尾自动跳过 padding 字节（F-order 跳过 axis 0 尾部填充，C-order 跳过 last axis 尾部填充），对外表现为仅遍历逻辑元素。**实现 `DoubleEndedIterator`**（反向遍历算法见下方 `PaddedContiguousIter` 算法说明）。用于 SIMD 优化的核心场景——padded 数组是最需要向量化遍历的场景。内部需记录 shape 和 padded strides 以正确跳跃 |
...
反向遍历 padded 数组须正确跳过行/列尾 padding，比"维护起止指针"复杂。以下是通用 N 维算法（以 C-order 为例，F-order 将 `last_axis` 替换为 `axis 0`）：

**2D C-order 示例**：`shape=[R, K]`，padded stride `[K_padded, 1]`（`K_padded ≥ K`）

```
状态：ptr（当前前向指针），back_ptr（当前反向指针），
      row_start（当前行起始），row_end（当前行逻辑末尾），
      remaining（剩余逻辑元素数）
...
next_back():
    ...
    if back_ptr < row_start:
        // 跨越到上一行：须跳过上一行尾部 padding
        back_ptr = row_start - (padded_stride - shape[last_axis]) * size_of::<A>() - size_of::<A>()
        row_start -= padded_stride[last_axis] * size_of::<A>()
        row_end = row_start + shape[last_axis] * size_of::<A>()
...
**3D C-order 示例**：`shape=[P, R, K]`，padded strides `[R*K_padded, K_padded, 1]`

3D 及更高维度引入**多层 padding 跳跃**：不仅需跳过最内层行尾 padding，还需跳过"平面间"padding（即每 P 个平面之间，由于最内层 padding 导致的额外字节）。算法扩展如下：

```
状态：ptr, back_ptr, remaining（同上）
      inner_row_start, inner_row_end（最内层行边界）
      plane_start（当前平面起始）
...
next_back():
    ...
    // 第一层：检查是否跨越最内层行边界
    if back_ptr < inner_row_start:
        // 跳过上一行尾部 padding（同 2D 算法）
        back_ptr = inner_row_start - (K_padded - K) * size_of::<A>() - size_of::<A>()
        inner_row_start -= K_padded * size_of::<A>()
        inner_row_end = inner_row_start + K * size_of::<A>()

        // 第二层：检查是否跨越平面边界
        // 当 inner_row_start 回退到上一平面范围外时触发
        if inner_row_start < plane_start:
            // 跳过上一平面的最后一行 padding
            // 上一平面起始 = plane_start - R * K_padded * size_of::<A>()
            plane_start -= R * K_padded * size_of::<A>()
            inner_row_start = plane_start + (R - 1) * K_padded * size_of::<A>()
            inner_row_end = inner_row_start + K * size_of::<A>()
            back_ptr = inner_row_end - size_of::<A>()
...
关键点：
- 行边界计算须使用 `padded_stride[last_axis]`（而非 `shape[last_axis] * size_of::<A>()`），因为 padded stride 包含 padding 字节
- 严格连续数组退化为普通 slice 迭代器（无 padding 跳跃），`next_back()` 零额外开销
- F-order 数组逻辑相同，仅将 `last_axis` 替换为 `axis 0`（列优先最内层维度）
...
| 宽松连续（有 padding） | `is_padded_contiguous()` = true 且 `is_contiguous()` = false | `None` | `Some(PaddedContiguousIter)` | `Iter` | 行间有 padding 字节，slice 迭代器不适用。优先使用 `iter_padded_contiguous()`（支持 `DoubleEndedIterator`） |
...
| `Item` | `(D::Pattern, &'a A)` |
| 遍历顺序 | 始终按 C-order（最右轴变化最快）产出 `(index, element)` 对。索引按逻辑坐标 C-order 递增：`(0,0,0) → (0,0,1) → … → (shape[0]-1, shape[1]-1, shape[2]-1)`，元素按对应逻辑位置取出（可能产生非连续访问）。**设计理由**：与 NumPy `ndenumerate`、ndarray `indexed_iter` 行为一致，保证遍历顺序的确定性（不受物理内存布局影响）。对于 F-contiguous 数组，访问模式为非连续的，但索引序列可预测且与用户直觉一致 |
| 长度 | `self.len()` |
| trait 实现 | `Iterator` + `ExactSizeIterator` + `FusedIterator` |
...
- 索引始终按 C-order 逻辑坐标递增（最右轴变化最快），不受物理内存布局影响。这保证索引 `(0, 0, 0) → (0, 0, 1) → ... → (shape[0]-1, shape[1]-1, shape[2]-1)` 的确定性顺序。元素按对应逻辑坐标寻址取出，不保证物理内存顺序访问
...
| 非连续输入 | 支持非连续输入，使用步长跳跃遍历（标量路径）。不自动拷贝——调用方若需最佳性能，应先确保输入连续（`is_padded_contiguous()`，见 §7.4）以触发 SIMD 路径。**注意**：批量运算（`batch_matvec`、`batch_dot`，见 §13.4）对非连续输入的行为不同——自动拷贝为 F-contiguous 后执行，可能产生 O(n) 隐式拷贝开销 |
| 填充输入 | 支持填充数组（见 §7.6），使用 stride 信息直接计算，填充区域的元素不参与运算 |
| 输出布局 | F-contiguous（BLAS 兼容输出，列优先。与库默认布局 C-order 不同，见 §7.1。此设计因 matvec 通常委托 BLAS 实现，BLAS 原生产出 F-contiguous 布局）。**注意**：F-contiguous 输出可能影响后续操作——如 `matvec` 结果直接 `reshape` 时，reshape 判定为 F-contiguous（非 C-contiguous），零拷贝路径的 strides 方向与 C-contiguous 不同。若需 C-contiguous 输出以保持与库默认布局一致，可在调用后使用 `.as_c_contiguous()`（见 §22）转换 |
...
| 错误处理 | **所有错误场景均 panic**（不返回 Result）。理由：`dot` 返回裸标量 `A`，签名无法返回 `Result<A, XenonError>`。这与 `outer`（返回 Tensor，可返回 Result）不同。**可恢复错误变体**：提供 `try_dot(&self, other: &Tensor<A, Ix1>) -> Result<A, XenonError>`，在维度/形状不匹配时返回 `Err` 而非 panic。`try_dot` 的语义与 `dot` 完全一致，仅错误处理方式不同。生产环境推荐使用 `try_dot` |
| 维度约束 | self 和 other 均须为 1D，否则 **panic**（`InvalidShape`）。`try_dot` 对应场景返回 `Err(InvalidShape)` |
| 形状约束 | self 和 other 长度须相等，否则 **panic**（`ShapeMismatch`）。`try_dot` 对应场景返回 `Err(ShapeMismatch)` |
| 空数组 | self 或 other 长度为 0 时返回 `A::zero()`（与 `sum` 对空数组的行为一致，因 `dot` 语义上等价于 `(self * other).sum()`） |
| 非连续输入 | 支持非连续输入，使用步长跳跃遍历（标量路径）。不自动拷贝——调用方若需最佳性能，应先确保输入连续以触发 SIMD 路径。**注意**：批量运算 `batch_dot`（见 §13.4）对非连续输入自动拷贝为 F-contiguous，行为与此不同 |
...
| 输出布局 | F-contiguous（BLAS 兼容布局，见 §25.3）。与 matvec 一致，输出与库默认 C-order 不同——后续 reshape 等操作需注意 strides 方向 |
...
| 方法签名 | `fn reshape<D2>(&self, shape: D2) -> Tensor<A, D> where D2: IntoDimension<Dim = D>`（编译时 ndim 校验，无 `-1` 支持）；`fn reshape_infer(&self, shape: &[isize]) -> Result<Tensor<A, D>>`（运行时 `-1` 推导）。两个方法均保持维度类型 `D` 不变。`reshape` 直接返回 `Tensor<A, D>`（形状不合法时 panic）；`reshape_infer` 返回 `Result`（`-1` 推导失败时返回 `Err(InvalidShape)`） |
...
| 输出维度 | ndim + 1 |
| 新轴步长 | **设计选择**：新轴步长设为 `product_of_subsequent_dims`（后续维度乘积）。例如对 C-contiguous `[3,4]`（strides=[4,1]），`unsqueeze(0)` 得到 `[1,3,4]`（strides=[12,4,1]）；对 F-contiguous `[3,4]`（strides=[1,3]），`unsqueeze(2)` 得到 `[3,4,1]`（strides=[1,3,12]）。**此策略保持连续性**：`is_c_contiguous()` / `is_f_contiguous()` 在 unsqueeze 前后一致，BLAS 兼容性不变。**与广播步长的区别**：广播使用 stride=0（语义为"重复同一元素"），但 stride=0 会导致 `HAS_ZERO_STRIDE=true`，从而使 `as_*_contiguous()` 返回 `None`、`is_blas_compatible()` 返回 `false`。unsqueeze 的 `product_of_subsequent_dims` 策略避免了此问题，因为新轴 size=1 时任何步长值在逻辑上等价（仅访问一个元素），选择非零步长可维持连续性标记的准确性。**代价**：stride 值可能与 `product_of_subsequent_dims` 计算结果不一致（当数组本身非连续时），但此时 unsqueeze 后仍为非连续，不引入新的退化 |
...
#### 对齐构造（from_vec_aligned / from_shape_vec_aligned）

当用户从现有 `Vec<A>` 构造 Tensor 且需要 SIMD 优化时，上述 `from_vec`/`from_shape_vec` 因复用 Vec 分配而无法保证 64 字节对齐（ALIGNED = false）。以下变体提供对齐保证：

| 属性 | 行为 |
|------|------|
| from_vec_aligned 签名 | `fn from_vec_aligned(v: Vec<A>) -> Tensor1<A> where A: Element` |
| from_shape_vec_aligned 签名 | `fn from_shape_vec_aligned<D: Dimension>(shape: D, v: Vec<A>) -> Result<Tensor<A, D>, ShapeError> where A: Element` |
| 行为 | 将输入 `Vec<A>` 的数据**拷贝**到新的 64 字节对齐缓冲区（不复用原 Vec 分配）。ALIGNED 标志为 `true` |
| 性能开销 | 一次额外内存拷贝 + 对齐分配。适用于 Tensor 构建后长期使用（如多次运算）的场景，摊销拷贝开销 |
| 与 from_vec 的关系 | `from_vec` 零拷贝但无 SIMD 优化；`from_vec_aligned` 有拷贝但保证 SIMD 优化。用户根据使用模式选择 |
| 形状验证 | `from_shape_vec_aligned` 与 `from_shape_vec` 相同的形状验证逻辑 |
...
**返回维度说明**：`is_close` 返回 `Tensor<bool, D>`（与 self 同维度类型），输出形状为 self 与 other 广播后的形状（见下方广播规则）。**广播维度约束**：广播后结果的 ndim 须等于 `D::NDIM`（静态维度在运行时校验，不匹配则返回 `ShapeError`；IxDyn 无此约束）。这意味着 `other` 的 ndim 不得大于 `self` 的 ndim（否则广播结果 ndim 超出 `D` 的表达范围）。**Strides 重计算**：输出 Tensor 为新分配的 Owned 数组（非视图），其 strides 按 C-contiguous 顺序从广播后形状重新推导，不继承输入的零步长 |
...
严格连续性方法（§22.2）使用 `is_f_contiguous()`/`is_c_contiguous()` 判定，将填充数组视为非连续。对于默认构造的张量（使用 SIMD 填充分配），这意味着 `as_*_contiguous()` 始终返回 `None`，`to_*_contiguous()` 始终拷贝。为避免此开销，提供宽松连续性变体：

**`as_*_padded` 系列（宽松零拷贝视图路径）**
...
| `as_c_padded_contiguous()` | `fn as_c_padded_contiguous(&self) -> Option<TensorView<'_, A, D>>` | 若输入满足宽松 C-连续性（`is_c_padded_contiguous()`），返回 `Some(TensorView)`；否则 `None` |
...
| Complex | 使用 Complex 的 Display 格式（见 §5.2），如 `1+2i`、`3-4i`、`0+1i`。**设计选择**：纯实数显示为 `3+0i`（而非 `3`），纯虚数显示为 `0+1i`（而非 `i`），零显示为 `0+0i`——始终保持 `re+imi` 格式以确保解析时结构可预测。此选择牺牲了简洁性换取了格式一致性，与 NumPy 的 `str()` 行为不同（NumPy 省略零部） | 使用 Complex 的 Debug 格式（见 §5.2），如 `Complex { re: 1.0, im: 2.0 }` |
