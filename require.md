...
| `type Smaller` | 关联类型 | 移除一个轴后的维度类型（Ix1 → Ix0, Ix2 → Ix1, ..., IxDyn → IxDyn）|
...
**步长存储说明**：shape 与 strides 共用同一维度类型 `D`。内部均以 `usize` 存储，但 strides 在运算时按 `isize` 解释（通过 `as isize` 转换），以支持反转轴的负步长。返回 `&[isize]` 切片时，将内部 `usize` 逐个转为 `isize`。这避免了为每个维度类型额外维护一个 `Stride` 关联类型，简化类型系统。

**Trait bound**：`Dimension: Debug + Clone + Eq + Hash + Send + Sync`

### 3.3 对应关系

| 维度类型 D | D::Pattern | D::Smaller | 实现 RemoveAxis |
|------------|------------|------------|-------------------|
| Ix0 | `()` | — | ✗（Ix0 无轴可移除） |
| Ix1 | `usize` | Ix0 | ✓ |
| Ix2 | `(usize, usize)` | Ix1 | ✓ |
| Ix3 | `(usize, usize, usize)` | Ix2 | ✓ |
| Ix4~Ix6 | `(usize, ..., usize)` | Ix(N-1) | ✓ |
| IxDyn | `Vec<usize>` | IxDyn | ✓ |

### 3.4 RemoveAxis trait

`RemoveAxis` 是独立的 trait，仅由可降维的维度类型实现。**Ix0 不实现此 trait**，从编译时保证零维张量不会调用 `remove_axis`。

| 方法 | 签名 | 说明 |
|------|------|------|
| `remove_axis()` | `&self, axis: usize -> D::Smaller` | 移除指定轴，返回降维后的维度 |

**Trait bound**：`RemoveAxis: Dimension<Smaller = Self::Smaller>`
