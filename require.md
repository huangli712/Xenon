...
| 操作 | 方法名 | 适用元素类型 | Trait Bound | 语义 |
|------|--------|-------------|-------------|------|
| 绝对值 | `abs` | 整数、浮点 | `A: Numeric` | \|x\|；`abs(MIN)` panic；`abs(-0.0)` → `0.0` |
| 模（复数） | `norm` | Complex | `A: ComplexScalar` | 返回 `Tensor<A::Real, D>`，值为 \|z\|。复数类型不提供 `abs`，使用 `norm` 代替 |
| 符号函数 | `signum` | 整数、浮点、复数 | `A: Numeric` | 正→1，零→0，负→-1；浮点传播 NaN；复数：z / \|z\|，零时返回零 |
| 平方 | `square` | 整数、浮点、复数 | `A: Numeric` | x \* x；整数溢出遵循 §12.7 |

**一元运算符**（见 §20.1）：

| 运算符 | 适用元素类型 | Trait Bound | 说明 |
|--------|-------------|-------------|------|
| `-` (Neg) | 整数、浮点、复数 | `A: Numeric` | 逐元素取反 |
| `!` (Not) | 仅 bool | — | 逻辑非 |
...
| 操作 | 方法名 | NaN/Inf 行为 |
|------|--------|-------------|
| 正弦 | `sin` | NaN → NaN；±Inf → NaN |
| 平方根 | `sqrt` | NaN → NaN；负数 → NaN；+Inf → +Inf |
| 指数 | `exp` | NaN → NaN；+Inf → +Inf；-Inf → 0.0 |
| 自然对数 | `ln` | NaN → NaN；0 → -Inf；负数 → NaN；+Inf → +Inf |
| 下取整 | `floor` | NaN → NaN；±Inf → ±Inf |
| 上取整 | `ceil` | NaN → NaN；±Inf → ±Inf |

**复数张量运算**（需 `A: ComplexScalar`，见 §4.6）：

| 操作 | 方法名 | 返回元素类型 | 语义 |
|------|--------|-------------|------|
| 共轭 | `conj` | `A`（Complex） | 逐元素共轭 z̄ |

> **命名规则**：复数模使用 `norm` 而非 `abs`。
...
| 规则 | 说明 |
|------|------|
| 一元运算 | 返回 `Tensor<A, D>`（新分配 Owned），元素类型不变 |
| 比较运算 | 返回 `Tensor<bool, D>` |
| norm | 返回 `Tensor<A::Real, D>` |
| 算术二元运算 | 返回 `Tensor<A, D>`（新分配或复用存储，见 §20.2） |
| 广播后输出形状 | 由广播规则推导（见 §16） |
...
**标量运算**（`_s` 后缀方法）：提供 `add_s`、`sub_s`、`mul_s`、`div_s` 等方法，标量视为 0D 张量广播到 self 形状。

### 12.6 广播行为

逐元素运算的广播遵循 §16 定义的 NumPy 风格广播规则。
...
| 场景 | 行为 |
|------|------|
| 加减乘溢出 | debug 模式 panic / release 模式 wrapping |
| 整数除零 | panic |
| abs(MIN) | panic |
| square 溢出 | 同加减乘溢出策略 |

> 归约运算的溢出策略与逐元素运算不同，见 §14.3。
