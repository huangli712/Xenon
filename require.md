...
| 输出布局 | F-contiguous（BLAS 兼容输出，列优先。与库默认布局 C-order 不同，见 §7.1。此设计因 matvec 通常委托 BLAS 实现，BLAS 原生产出 F-contiguous 布局） |
...
| `nanargmin_axis_checked` | `fn nanargmin_axis_checked(&self, axis: usize) -> Tensor<Option<usize>, D::Smaller>` | 沿轴版本，每个切片独立返回 `Option<usize>` |
| `nanargmax_axis_checked` | `fn nanargmax_axis_checked(&self, axis: usize) -> Tensor<Option<usize>, D::Smaller>` | 同上 |

**注意**：`_checked` 沿轴版本返回 `Tensor<Option<usize>, D::Smaller>`，其中 `Option<usize>` 不实现 `Element` trait，因此这些方法通过专用构造器（非标准 `Tensor::from_shape_vec`）创建。实现上可使用 `Tensor<MaybeUninit<usize>, D::Smaller>` + 有效性掩码跟踪，最终通过 `unsafe` 转换为 `Tensor<Option<usize>, D::Smaller>`。此设计权衡：牺牲 `Element` 约束一致性，换取生产环境可恢复安全性。
...
| 一元运算 | Neg, Not | 取反、逻辑非（`Not` 对 `bool` 为逻辑非，对整数类型为按位取反） |
| 广播 | 所有二元运算符 | 隐式支持广播 |
...
| `CowTensor<'a, A, D>` | 类型别名 | `TensorBase<CowRepr<'a, A>, A, D>` |
