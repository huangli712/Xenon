...
| IxDyn | 动态维度 | 堆分配 `Vec<usize>` | `From<Vec<usize>>` / `From<&[usize]>` |
...
| `type Larger` | 关联类型：`Dimension + RemoveAxis` | 插入一个轴后的维度类型（Ix0 → Ix1, ..., Ix5 → Ix6, Ix6 → IxDyn, IxDyn → IxDyn）。所有维度类型均可升维，因此定义在 `Dimension` 而非独立 trait。约束 `RemoveAxis` 确保 `insert_axis()` 的返回值可直接调用 `remove_axis()`（升维→降维为常见操作链，如 `stack` 后再 `collapse`），避免泛型代码中手动添加 `D::Larger: RemoveAxis` bound |
| `ndim()` | `&self -> usize` | 返回维度数。静态维度（Ix0~Ix6）时返回编译时常量；IxDyn 为普通函数 |
| `slice()` | `&self -> &[usize]` | 返回 shape 切片 |
| `slice_mut()` | `&mut self -> &mut [usize]` | 返回可变 shape 切片。仅供内部使用（reshape 等原地修改 shape 的操作），外部用户不应直接修改 Tensor 正在使用的 Dimension 的 shape |
| `size()` | `&self -> usize` | 返回总元素数（各轴乘积）。Ix0（空维度）返回 1（空乘积的数学约定）。溢出时 debug 模式 panic，release 模式 wrapping |
...
| `default_strides()` | `&self -> Self` | 计算 F-order 默认步长，以 `usize` 存储于 `Self` 内部 |
| `default_strides_c()` | `&self -> Self` | 计算 C-order 默认步长，以 `usize` 存储于 `Self` 内部 |
| `into_dyn()` | `&self -> IxDyn` | 将任意维度类型转换为动态维度 `IxDyn`。静态维度（Ix0~Ix6）将内部数组数据拷贝到 `Vec<usize>`（堆分配）；`IxDyn` 为 identity（零开销）。此为静态→动态维度转换的唯一入口，用于需要运行时维度类型的场景（如动态 unsqueeze、跨维度类型操作）。转换总是成功（infallible）。反向转换（动态→静态）使用标准库 `TryFrom<IxDyn>` trait，失败时返回 `DimensionMismatch`（见 §3.5） |

**步长存储设计**：

shape 与 strides 共用同一维度类型 `D`，内部均以 `usize` 存储。步长在运算时按 `isize` 解释——因 `usize` 与 `isize` 内存布局等价（同为 pointer-sized integer、二进制补码，所有 Rust 支持的平台上 `size_of` 和 `align_of` 相等），`strides()` 返回 `&[isize]` 时为零成本 transmute，无逐元素转换开销。负步长存储为 `usize` 的二进制补码表示（如 `-3isize` 存储为 `usize::MAX - 1`），按 `isize` 解释时自动恢复负值。`Debug` 实现须将 strides 以 `isize` 形式输出（而非原始 `usize`），以确保调试可读性。
...
| 转换方向 | API | 所在位置 | 说明 |
|----------|-----|----------|------|
| 静态 → 动态 | `dim.into_dyn()` | `Dimension` trait 方法（见 §3.2） | Infallible，返回 `IxDyn`。静态维度拷贝数组数据到 `Vec`（堆分配），`IxDyn` 为 identity |
...
- Ix6 不可 `unsqueeze`（已达最大静态维度，且 reshape 不改变维度类型）。`Ix6::Larger = IxDyn` 表示类型系统能表达增长，但 `unsqueeze` 保持维度类型不变，因此 Ix6 无法 unsqueeze。需要插入轴时，须先 `into_dyn()` 转换为 `IxDyn` 再操作
...
| Complex / Complex | `(a+bi)/(c+di) = ((ac+bd) + (bc-ad)i) / (c²+d²)`。分母为零时返回 NaN + NaN\*i |
...
| 存储模式 | `Storage` | `StorageMut` | `capacity()` | `as_ptr()` | `as_mut_ptr()`（via StorageMut） | `is_owned()` | `Clone` |
|----------|-----------|--------------|--------------|------------|----------------------------------|--------------|---------|
...
采用 6 标志位设计（1 字节 u8，低 6 位有效，高 2 位保留，须为 0），缓存高频查询的派生状态：
...
| HAS_ZERO_STRIDE | bit 3 | 零步长（缓存） | 存在步长为 0 的维度。两种来源：(1) 广播产生的 size-1 维度（逻辑上重复该元素）；(2) unsqueeze 插入的 size-1 轴（§17.8，步长设为 0 以保持连续性）。优化路径据此判断不可按简单步长递增遍历，但连续性标志不受影响。**注意：当 shape 含零维度（任意 axis 的 shape[k]==0）时，该标志无意义（无元素被访问），此时所有布局标志值均为未定义** |
...
| 小数组优化 | 当 `元素数 × size_of::<A>() ≤ 64` 时，允许降级到 `max(align_of::<A>(), 8)` 字节对齐（即元素自然对齐，但至少 8 字节以保证基本 SIMD 安全）。降级后 ALIGNED 标志为 false。阈值 64 与默认对齐一致，避免小数组浪费内存 |
...
`M_padded = ⌈M × size_of::<A>() / alignment⌉`（向上取整到 alignment 字节边界），默认 alignment = 64 字节。
...
| make_mut (ArcRepr) | 不保持 | 新分配使用默认对齐（64 字节），但不添加填充 |
...
| flags: u8 | 布局标志位（低 6 位有效，高 2 位保留为 0）。缓存高频查询的派生状态（F/C 连续性、对齐、零步长、负步长、填充），使布局查询方法达到 O(1) 复杂度（见 §7.3 标志定义与 §7.4 查询方法）。创建时根据分配方式初始化，切片/转置/reshape 时重新计算，视图创建时继承并按需降级 |
...
| `layout_flags()` | `&self -> u8` | 返回完整布局标志原始值 |
...
| 操作类别 | 示例 | SIMD 要求 |
|----------|------|-----------|
| 逐元素一元 | abs, neg, sqrt, exp, ln | 宽松连续内存（`is_padded_contiguous()`，见 §7.4）；不满足时回退标量路径 |
| 逐元素二元（浮点/复数） | add, sub, mul, div | 两操作数均宽松连续内存（`is_padded_contiguous()`）且形状兼容；任一操作数不满足时回退标量路径 |
| 逐元素二元（整数） | add, sub, mul | 两操作数均宽松连续内存（`is_padded_contiguous()`）且形状兼容；任一操作数不满足时回退标量路径 |
| 逐元素二元（整数除法） | div（整数类型） | 不走 SIMD 路径，始终标量。理由：主流 SIMD 指令集（AVX2/SSE/NEON）不提供整数除法指令，pulp 亦不提供整数 SIMD 除法；即使连续内存也回退标量路径 |
| 归约（浮点/复数） | sum, prod, min, max | 宽松连续内存（`is_padded_contiguous()`）；不满足时回退标量路径 |
| 归约（整数 sum/prod） | sum, prod | 不走 SIMD 路径，始终标量。理由：整数 checked 算术（§14.3）要求每步检测溢出，SIMD 指令集无对应操作 |
| 归约（整数 min/max） | min, max | 宽松连续内存（`is_padded_contiguous()`）；不满足时回退标量路径 |
| 内积 | dot | 宽松连续内存（`is_padded_contiguous()`） |
| 比较 | eq, lt, gt（生成 mask） | 宽松连续内存（`is_padded_contiguous()`）；不满足时回退标量路径 |
...
|------|----------|
| 元素数 < SIMD 宽度 | 标量路径 |
| 元素数 ≥ SIMD 宽度 且 宽松连续内存（`is_padded_contiguous()`） 且 simd 启用 且 操作有 SIMD 实现（见注） | SIMD 路径 |
| 元素数 ≥ 并行阈值 且 parallel 启用 且 宽松连续内存（`is_padded_contiguous()`） 且 simd 启用 且 操作有 SIMD 实现 | SIMD + 并行路径（每线程内部使用 SIMD） |
| 元素数 ≥ 并行阈值 且 parallel 启用 且 非宽松连续内存 | 标量 + 并行路径（每线程使用标量迭代，按块分割非连续数据。宽松连续不是并行路径的前提条件——非连续数据也可并行，只是每线程内部不走 SIMD） |
| simd 未启用 且 元素数 ≥ 并行阈值 且 parallel 启用 | 标量 + 并行路径 |
| 非宽松连续内存 且 元素数 < 并行阈值（或 parallel 未启用） | 标量路径（单线程，步长跳跃遍历） |
...
| zip 形状不一致但可广播 | 自动广播后迭代，等价于先 broadcast 再 zip。广播视图使用零步长实现逻辑重复（无额外堆分配），步长为 0 的轴在迭代时重复产出同一位置的数据。**遍历顺序规则**：若未指定遍历顺序，使用库默认 F-order（广播视图本身无物理内存，"物理布局顺序"在此场景下无意义，统一使用 F-order 保证行为确定性）。若显式指定遍历顺序（F 或 C），则按指定逻辑顺序迭代，可能产生非连续访问 |
...
| 动态维度（IxDyn） | `SmallVec<[usize; 8]>` 或等效内联缓冲区（≤ 8 维栈上，> 8 维回退堆） | 通常无（6 维以内零堆分配） |
...
| `iter_contiguous()` | `fn iter_contiguous(&self) -> Option<ContiguousIter<'_, A>>` | `&A` | 所有存储模式 | 仅连续数组返回 `Some`。`ContiguousIter` 实现 `DoubleEndedIterator` + `ExactSizeIterator` + `FusedIterator`。非连续数组返回 `None` |
| `iter_contiguous_mut()` | `fn iter_contiguous_mut(&mut self) -> Option<ContiguousIterMut<'_, A>> where S: StorageMut` | `&mut A` | Owned, ViewMutRepr | 可变连续迭代。同上检查 `has_zero_stride()` 和 `S: StorageMut` |
...
- `iter()` → `Iter`：通用迭代器，支持所有数组（连续与非连续），**不实现 `DoubleEndedIterator`**
- `iter_contiguous()` → `Option<ContiguousIter>`：仅连续数组可用，**实现 `DoubleEndedIterator`**（底层为 slice 迭代，`next_back()` 零开销）
...
- IxDyn 的 `D::Pattern = Vec<usize>`，每次 `next()` 产出都会分配一个 `Vec`。性能敏感场景下，建议使用 `iter()` 配合手动坐标计算（或使用 `enumerate()` 获取展平索引）
...
| 绝对值（复数） | `abs` | Complex | `A: ComplexScalar` | 返回 `Tensor<A, D>`，值为 `Complex(norm, 0)`（实部为模，虚部为 0），保持元素类型不变。若需纯实数模值，使用 `norm()` 方法 |
...
| 方法 | 签名 | 说明 |
|------|------|------|
| `mapv_into` | `fn mapv_into<F>(self, f: F) -> Tensor<A, D> where F: Fn(A) -> A` | 消耗 self，逐元素应用 `f`，结果写回原存储（前提：连续内存）。等价于 ndarray 的 `mapv_into`。适用于链式调用中复用存储，如 `t.mapv_into(|x| x * 2.0)` |
...
| `mapv_mut` | `fn mapv_mut<F>(&mut self, f: F) where F: Fn(A) -> A` | 原地映射，等价于 `self.mapv_into(f)` 但不消耗所有权（`&mut self`）。须检查 `has_zero_stride()`（见 §11.3） |
...
| 场景 | 行为 | 说明 |
|------|------|------|
| 加减乘溢出 | **panic**（debug） / **wrapping**（release） | 与 Rust 默认行为一致：debug 模式使用 `checked` 语义（溢出 panic），release 模式使用 `wrapping` 语义（溢出回绕）。**不使用** `saturating` 语义 |
...
**argmin/argmax NaN 行为**：浮点数组含 NaN 时，`argmin`/`argmax` 使用 `PartialOrd` 的比较语义。由于 NaN 与任何值比较均返回 `false`（`NaN < x` 和 `NaN > x` 均为 `false`），NaN 不参与大小比较。若所有元素均为 NaN，`argmin`/`argmax` 返回第一个 NaN 的索引（按遍历顺序）。若数组同时包含非 NaN 和 NaN 元素，非 NaN 元素正常比较，NaN 被跳过——但具体行为依赖遍历实现，建议调用方先通过 `.filter()` 或 `is_nan()` mask 排除 NaN。NaN 忽略变体 `nanargmin`/`nanargmax` 始终跳过 NaN（见 §14.4）。
...
| `nanargmin` | 跳过 NaN 返回最小值索引，全 NaN 或空数组返回 `NaN`（注意返回类型为 `A` 而非 `usize`，以表达"无有效索引"） | `A`（全局）或 `Tensor<A, D::Smaller>`（沿轴） |
| `nanargmax` | 跳过 NaN 返回最大值索引，行为同 `nanargmin` | 同上 |

**设计说明**：`nanargmin`/`nanargmax` 在全 NaN 或空数组时无法返回有效的 `usize` 索引（`usize` 无 NaN 表示），因此返回 `A` 类型。非 NaN 场景下，返回值可安全 `as` 转为 `usize`（值在有效索引范围内）。调用方须检查返回值是否为 NaN 以判断有效性。
...
| `usize` 不实现 | `usize` 定位为索引返回类型（§4.2），不作为数据值参与集合操作 |
...
| 属性 | 行为 |
|------|------|
| bins 参数 | 整数（等宽 bin 数）或 `Tensor1<A>`（自定义 bin 边界数组） |
...
**公共工具函数**：提供内部函数 `broadcast_shape(shape_a: &[usize], shape_b: &[usize]) -> Result<SmallVec<[usize; 8]>, BroadcastError>`，供所有需要广播形状推导的操作（逐元素运算、zip、batch 操作）复用。

**算法**：

```
fn broadcast_shape(sa: &[usize], sb: &[usize]) -> Result<SmallVec<usize, 8>, BroadcastError> {
...
| 操作 | 返回维度类型 | 关联类型 | 约束 |
|------|-------------|----------|------|
| unsqueeze | `D::Larger` | `Dimension::Larger`（§3.2） | Ix6 不可调用（Ix6::Larger = IxDyn，与"保持维度类型"矛盾），编译错误 |
| stack | `D::Larger` | `Dimension::Larger`（§3.2） | Ix6 同理，编译错误。IxDyn → IxDyn（ndim + 1） |
...
| 多轴挤压 | `squeeze_axes(axes: &[usize])` 同时消除多个 size-1 轴。多轴挤压的返回类型通过递归应用 `RemoveAxis` 关联类型推导（如 Ix3 squeeze 2 轴 → Ix1），仅支持 IxDyn 的多轴挤压在签名中返回 `IxDyn` |
...
| 静态维度约束 | Ix6 不可 unsqueeze（Ix6::Larger = IxDyn，与"保持静态维度类型"矛盾），编译错误。IxDyn 无此限制 |
...
| Ix6 上限 | 对 Ix6 数组调用 stack 时，`Ix6::Larger = IxDyn`，编译错误（stack 保持维度类型不变的要求与 Ix6::Larger = IxDyn 矛盾，见 §17.0）。如需在 Ix6 上堆叠，须先 `into_dyn()` 转为 IxDyn |
...
| 属性 | 行为 |
|------|------|
| 签名 | `fn select(condition: &Tensor<bool, D>, x: &Tensor<A, D>, y: &Tensor<A, D>) -> Tensor<A, D>`（三者形状须一致或可广播为同一形状） |
...
| 返回维度 | 输出维度类型为三者广播后的维度类型：三者均为静态维度时保留静态类型（须一致）；任一为 IxDyn 时输出 IxDyn |
| 返回类型 | 新分配的 Owned 数组 |
...
| UninitTensor 定义 | 独立结构体，内部持有 `shape: D`、`strides: D`、`offset: usize`、`flags: u8` 和对齐堆分配的 `MaybeUninit<A>` 缓冲区。不实现 `Storage` trait（因 `MaybeUninit<A>` 不满足 `Element`），不暴露任何读取方法 |
...
| 对齐 | 使用默认 64 字节对齐分配 |
| 布局 | 默认 F-contiguous（与项目全局布局一致，见 §7.1） |
...
| 布局 | 默认 F-contiguous（与 §7.1 全局布局一致），步长由形状按 F-order 推导 |
...
| 返回类型 | 新分配的 Owned 数组 |
| 元素为 NaN | NaN 比较为 false，clip 不修改 NaN（NaN 不满足 min/max 约束，保持不变） |
| min 为 NaN | panic（调用方违反前置条件）。理由：`x < NaN` 恒为 false，导致下限裁剪失效且行为依赖实现方式（使用 `f64::min` 则 NaN 传播使结果全为 NaN，使用 if-else 则下限失效）。显式 panic 比静默错误或平台相关行为更安全。与 "min > max → panic" 的前置条件检查一致 |
| max 为 NaN | panic（调用方违反前置条件）。理由同上：`x > NaN` 恒为 false，行为依赖实现且不可预测 |
| min 和 max 均为 NaN | panic（同上） |
...
**返回维度说明**：`is_close` 返回 `Tensor<bool, D>`（与 self 同维度类型），输出形状为 self 与 other 广播后的形状（见下方广播规则）。当广播后形状与 `self.shape()` 不同时，返回的 `D` 实例的内部值会被更新为广播后形状。这要求 `D: Dimension` 的 `slice_mut()` 方法可用于原地更新 shape（见 §3.2）。
...
| as_mut_ptr() | 返回数据起始位置的可变原始指针 `*mut A`，即 `storage.as_mut_ptr() + offset`（须可写存储）。FFI 调用方须确保不会越过 `storage.capacity() - offset` 边界写入 |
| as_ptr_unchecked() | unsafe，不检查偏移量有效性 |

### 25.2 形状与步长查询

`shape()` / `strides()` / `offset()` 在 §8.3 中定义，FFI 集成直接调用即可。此处仅补充 `strides_bytes()`，因为 `§8.3` 的 `strides()` 返回元素单位步长，而 C/BLAS 接口通常需要字节单位。
...
| 方法 | 说明 |
|------|------|
| lda() | 返回 leading dimension（F-order 下为第一轴步长），仅 2D 及以上。对 batch 视图（如 3D 数组在最后一维上的 2D 切片），lda() 返回该 2D 切片的第一轴步长，不受 batch 轴影响。填充数组的 lda() 自然返回填充后的物理步长（如 F-order M_padded），可直接传递给 BLAS 的 LDA 参数 |
| is_blas_compatible() | 检查内存布局是否可直接传递给 BLAS：宽松连续（`is_padded_contiguous()` 为 true，即严格 F/C-contiguous 或填充后的 F/C-padded-contiguous），正步长，无零步长。BLAS 的 LDA 参数天然支持 leading dimension 不等于逻辑维度，因此填充数组是 BLAS 兼容的 |
...
| 逐元素运算隐式广播 | `BroadcastError` | 通过 Zip 触发，含冲突轴诊断信息 |
