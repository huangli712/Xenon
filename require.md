...
| `size()` | `&self -> usize` | 返回总元素数（各轴乘积）。Ix0（空维度）返回 1（空乘积的数学约定）。**溢出时始终 panic**（debug 与 release 一致）。此方法用于所有需要元素计数的场景，避免 release 模式 wrapping 导致分配过小缓冲区。对于明确需要 wrapping 语义的场景，使用 `size_wrapping()` |
...
**步长值域约束**：所有步长的绝对值须 ≤ `isize::MAX as usize`。此约束保证：1）usize→isize transmute 后符号正确；2）任何以 usize 比较步长绝对值的代码不会因负步长的补码表示而产生错误判断。`TensorBase` 层面提供 `stride_isize(&self, axis: usize) -> isize` 方法，封装 transmute 逻辑，避免散落的 transmute 调用。
...
- **分配**：使用 `Layout::from_size_align(capacity * size_of::<A>(), alignment)` + `alloc::alloc()`。`alignment` 至少为 `max(64, align_of::<A>())`
...
| 方法 | 签名 | 说明 |
|------|------|------|
| `len()` | `&self -> usize` | 返回总元素数（各轴长度乘积），委托到 `self.shape.size()`，溢出行为与 `Dimension::size()` 一致（debug panic，release wrapping）。**契约**：对通过本库构造函数创建的 tensor，shape 乘积不溢出 usize（构造函数已验证 `size_checked()`，见 §8.1 (5)），`len()` 返回正确值；对通过 `from_raw_parts` 创建的 tensor，调用方须确保 shape 乘积不溢出。非分配场景可安全使用；分配场景须使用 `len_checked()` |
...
基于 pulp crate 的 x86 microarchitecture level 分层模型（V3/V4）和 aarch64 支持：

| 分层 | 架构 | Feature flag | 包含的指令集 | pulp dispatch 路径 |
|------|------|-------------|-------------|-------------------|
| V4 | x86_64 | `x86-v4`（须显式启用，见下方说明） | AVX-512（F/BW/CD/DQ/VL），向下兼容 V3 | `Arch::V4` |
| V3 | x86_64 | 默认启用 | AVX2 + FMA + BMI1/2 + LZCNT + SSE4.2 等（x86-v3 baseline） | `Arch::V3` |
| Neon | aarch64 | 默认启用 | NEON + 可选 FCMA | `Arch::Neon` |
| 标量 | 所有 | — | 无 SIMD | `Arch::Scalar` |

**重要说明**：

| 项目 | 说明 |
|------|------|
| 无独立 SSE4.2 路径 | 不支持 AVX2 的 x86 CPU（2012 年前）直接退化为标量。pulp 的 `Arch` enum 仅包含 `Scalar | V3 | V4`（x86），V2 不在 dispatch 路径中 |
| AVX-512 需显式 feature | pulp 的 `x86-v4` feature 默认不启用。使用 AVX-512 须同时启用 Xenon 的 `simd` feature 和 pulp 的 `x86-v4` feature（通过 Cargo.toml 的 `pulp = { features = ["x86-v4"] }` 或 Xenon 提供的子 feature `simd-avx512`） |
| WASM SIMD | pulp 支持通过 `wasm` feature 启用 WASM SIMD（`simd128` + `RelaxedSimd`）。当前版本未纳入 Xenon 的 SIMD 路径，后续版本可考虑支持 |
...
| 逐元素二元（整数） | add, sub, mul | 两操作数均宽松连续内存（`is_padded_contiguous()`）且形状兼容；任一操作数不满足时回退标量路径。**注**：pulp 的整数 SIMD `mul` 不支持 `u8`/`i8`（仅支持 u16/i16/u32/i32/u64/i64），`u8`/`i8` 乘法回退标量路径 |
...
| `nanargmin` | 跳过 NaN 返回最小值索引，全 NaN 或空数组返回 `None` | `Option<usize>`（全局）或 `Tensor<Option<usize>, D::Smaller>`（沿轴） |
| `nanargmax` | 跳过 NaN 返回最大值索引，行为同 `nanargmin` | 同上 |

**设计说明**：`nanargmin`/`nanargmax` 在全 NaN 或空数组时无法返回有效的 `usize` 索引，使用 `Option<usize>` 表达"无有效索引"的语义：`Some(index)` 为有效索引，`None` 表示无有效元素。这比使用 NaN 值（原设计）更符合 Rust 惯用法——整数类型无 NaN 表示，使用 Option 是表达可能不存在值的标准方式。返回 `Tensor<Option<usize>, D::Smaller>`（沿轴）时，每个位置独立判断是否有有效索引。
