...
**整数归约溢出行为**：`sum` 作用于整数数组时，溢出**始终 panic**（debug 和 release 模式均如此）。**此策略与 §12.7 逐元素运算的溢出策略不同**：归约运算涉及多步累加，wrapping 导致的错误值会传播到最终结果且难以诊断；逐元素运算的每步结果独立，wrapping 语义可预测且与 Rust 默认一致。
