...
| HAS_ZERO_STRIDE | bit 3 | 零步长（缓存） | 存在步长为 0 的维度。两种来源：(1) 广播产生的 size-1 维度（逻辑上重复该元素）；(2) unsqueeze 插入的 size-1 轴（§12.8，步长设为 0 以保持连续性）。优化路径据此判断不可按简单步长递增遍历，但连续性标志不受影响。**注意：当 shape 含零维度（任意 axis 的 shape[k]==0）时，该标志无意义（无元素被访问），此时所有布局标志值均为未定义** |
