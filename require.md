...
Element trait 为 sealed（防止外部实现），但提供 `unsafe impl Element` 的 escape hatch 供高级用户为自定义类型实现。详见§4.4。
...
bool 的 sum() 返回 usize（count of true 值），等价于 count_true()。bool 不支持 prod()。usize 支持所有 Numeric 操作。
...
| 可见部分优化 | 注意：当视图仅引用底层缓冲区的小部分（< 50%）时，make_mut 应仅克隆可见部分而非整个缓冲区，以避免不必要的内存拷贝。这与 ndarray 的 OwnedArcRepr 策略一致。
...
**CowRepr<'a, A>** — Copy-on-Write 存储表示。可以持有 owned 数据或 borrowed 视图，首次修改时按需克隆。适用于需要统一接受 owned/borrowed 输入的 API。CowRepr 的开销比 ArcRepr 更小（无 Arc 引用计数），适合单线程场景。
...
(3) **地址计算中间精度**：元素地址公式 `offset + Σ(index[k] * stride[k])` 中，单项 `index[k] * stride[k]` 的乘积在 64-bit 平台上可能超出 `isize` 范围（即使满足上述两个不变量，当 ndim ≥ 2 时累积乘积仍可能溢出）。实现须使用 `isize` 中间精度进行乘法与累加，构造函数须验证 shape×stride 不超出 isize 范围（constructor-time overflow validation），最终结果转为 `usize` 后验证 < 存储容量。不得依赖 `i128` 中间计算。
