...
| 返回类型 | 返回 `ScratchRequirement` 结构体（大小 + 对齐），而非裸 usize。定义：`pub struct ScratchRequirement { pub size: usize, pub alignment: usize }`，其中 `size` 为所需字节数，`alignment` 为最小对齐要求（须为 2 的幂，构造时通过 `debug_assert!` 验证） |
