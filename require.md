...
| 填充轴 | F-order 填充 axis 0（第一轴），C-order 填充 last axis（最后一轴）。填充后步长（strides）反映实际物理内存布局（含填充量），详见 §7.5 |
...
| PADDED | bit 5 | 主维度填充 | 数组的主维度（F-order 的 axis 0，C-order 的 last axis）经过填充以对齐 SIMD 访问。PADDED 为 true 时，步长反映填充后的物理布局（strides[1] ≥ shape[0] for F-order），但严格连续性标志（F_CONTIGUOUS / C_CONTIGUOUS）仍按无填充的 shape 乘积判定。SIMD 路径应使用宽松连续性检查（`is_f_padded_contiguous()` / `is_c_padded_contiguous()`，见 §7.3） |
...
- `PADDED && !F_CONTIGUOUS && !C_CONTIGUOUS`：填充数组（严格连续性为 false，但 SIMD 可通过宽松连续性路径处理，见 §7.3）
...
| flags: u8 | 布局标志位（低 6 位有效，高 2 位保留为 0）。缓存高频查询的派生状态（F/C 连续性、对齐、零步长、负步长、填充），使布局查询方法达到 O(1) 复杂度（见 §7.2 标志定义与 §7.3 查询方法）。创建时根据分配方式初始化，切片/转置/reshape 时重新计算，视图创建时继承并按需降级 |
...
**布局查询**（基于 `flags` 字段的 O(1) 查询，标志定义见 §7.2）
...
| 操作类别 | 示例 | SIMD 要求 |
|----------|------|-----------|
| 逐元素一元 | abs, neg, sqrt, exp, ln | 宽松连续内存（`is_padded_contiguous()`，见 §7.3）；不满足时回退标量路径 |
...
| 填充字节处理 | 填充数组的 SIMD 加载可能包含填充区域的字节（见 §7.5.5），这是安全的：读取无副作用；写入可能覆写填充字节（值变为未定义），但不影响逻辑元素 |
...
| 规则 | 要求 |
|------|------|
| SIMD 访问许可 | SIMD 操作可读取 padding 字节（无副作用，值可能非零但安全）；SIMD 写操作可能覆写 padding 字节（值变为未定义）。这是填充支持 SIMD 的必要条件（见 §7.5.5） |
...
| 写操作范围 | 逐元素写操作（fill, map_inplace, 复合赋值等）只写入逻辑元素范围，padding 值未定义（见 §7.5.5） |
...
- 广播产生的 size-1 维度使用零步长（见 §7.2 `HAS_ZERO_STRIDE` 标志）
...
| unsafe empty_uninit | 返回 `Tensor<MaybeUninit<A>, D>`，未初始化内存（见 §19.3 empty_uninit 语义） |
