...
| diag | `fn diag(diag: &Tensor1<A>) -> Tensor<A, Ix2>` | 以一维数组为对角线构造方阵，非对角位置为 `zero()` |
| diag_offset | `fn diag(diag: &Tensor1<A>, offset: isize) -> Tensor<A, Ix2>` | 偏移对角线：`offset > 0` 上移，`offset < 0` 下移；输出形状为 `(n + |offset|) × (n + |offset|)` |
...
| `mapv_into` | `fn mapv_into<B, F>(self, f: F) -> Tensor<B, D> where F: FnMut(A) -> B` | 消耗 Owned 数组，对每个元素按值（无需 clone）应用 `f`，可变换元素类型。适用于 Owned 数据的场景：按值传递比 `map`（接受 `&A`）避免了一次 clone，同时允许输出类型不同于输入类型（如 `f64 → i64`）。**仅对 `Tensor<A, D>`（Owned）实现**，View/ViewMut 无自有存储不可消耗。View/ViewMut 用户的替代方案：先 `.to_owned()` 转为 Owned 再调用 `mapv_into`，或直接使用 `map()`（代价为一次额外 clone） |
...
> 以上 `shape()` / `strides()` / `offset()` 的语义定义详见 §6.3，本节仅列出 FFI 集成使用方式。
...
| into_raw_parts | `fn into_raw_parts(self) -> (*const A, D, D::Stride, usize)` | 消耗数组，返回 (ptr, shape, strides, offset) 元组 |
