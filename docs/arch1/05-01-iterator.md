...
迭代器模块是 Xenon 张量库的数据遍历基础设施，为所有张量操作提供统一、高效、类型安全的遍历机制。它实现了多种迭代模式，支持从简单的元素遍历到复杂的多张量同步迭代。
...
`Zip` 是 Xenon 迭代器系统中最强大的组件，支持多个张量的同步迭代。它实现了 ndarray 风格的 Producer pattern，支持广播和并行化。
...
Producer pattern 是 ndarray 的核心设计，Xenon 采用相同模式：
