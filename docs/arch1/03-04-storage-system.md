...
存储系统是 Senon 张量库的基础设施层，负责管理张量数据的底层存储方式。核心职责包括：
...
Senon 采用 ndarray 风格的存储抽象，提供四种互补的存储模式：
...
参考 ndarray 的 `RawData → Data → DataMut → DataOwned` 层次，Senon 采用类似的分层设计：
...
**理由**: Storage traits 是 Senon 内部契约，不允许外部类型实现。Sealed trait 模式确保只有 Senon 定义的存储类型满足这些约束，维护 API 稳定性。
