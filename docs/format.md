...
| 2. 文件位置 | ✅ | 文件路径 + 划分理由 | §1.3 |
| 3. 依赖关系 | ✅ | 依赖图 + 具体消费的类型/trait | §5 |
| 4. 公共 API 设计 | ✅ | trait 定义、结构体、方法签名 | §6 |
| 5. 内部实现设计 | ✅ | 算法、数据结构、安全性论证 | §7 |
| 6. 实现任务拆分 | ✅ | Wave/Phase 分组，统一粒度 | §8 |
| 7. 测试计划 | ✅ | 单元测试 + 集成测试 + 属性测试 + 边界测试 | §9 |
| 8. 与其他模块的交互 | ✅ | 接口约定、数据流 | §10 |
| 9. 设计决策记录 | 推荐 | ADR 格式 | §11 |
| 10. 性能考量 | 推荐 | 复杂度、内存、缓存行为 | §12 |
| 11. no_std 兼容性 | 按需 | 条件编译说明 | — |
...
| 职责 | 包含 | 不包含 |
|------|------|--------|
| 元素遍历 | 按内存布局顺序遍历 | 具体运算逻辑（由 `ops/` 提供） |
...
| 产出（输出） | `ops/` | `TensorView<'a, A, D>` | 生命周期绑定原始张量 |
```
