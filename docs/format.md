# 实施方案文档规范

> 本文档定义项目中所有实施方案（architecture/design document）的编写规范。
> 每个模块的设计文档必须遵循本规范，以确保文档的一致性、可读性和可执行性。

---

## 1. 文件命名与编号

### 1.1 扁平全局编号

采用 `NN-模块名.md` 的扁平编号格式，NN 为两位全局递增序号：

```
00-rust-standards.md
01-architecture-overview.md
02-dimension.md
03-element-types.md
...
```

**优点**：引用时只需一个数字（"参见文档 10"），跨文档交叉引用简单。

### 1.2 模块分组通过目录或编号段体现

若模块数量较多，可通过编号段自然分组：

| 编号段 | 含义 |
|--------|------|
| `00-01` | 全局规范与架构概览 |
| `02-09` | 核心类型系统 |
| `10-19` | API 模块 |
| `20-21` | 计算后端 |
| `22-24` | 质量保障 |

### 1.3 文件位置声明

每个文档第 2 节必须声明文件位置：

```markdown
## 2. 文件位置

```
src/
├── dimension.rs       # 所有维度类型和 Dimension trait
```

单文件设计：维度类型之间高度相关，拆分反而增加耦合复杂度。
```

---

## 2. 文档元信息头

每个文档开头必须包含元信息块：

```markdown
# <模块名>模块设计

> 文档编号: NN | 模块: `src/<module>/` | 阶段: Phase X
> 前置文档: `00-rust-standards.md`, `01-architecture-overview.md`, ...
> 需求参考: 需求说明书 §X
```

**前置文档链**：明确列出阅读本文档前须先理解的文档，形成阅读顺序指引。

---

## 3. 标准章节结构

每个实施方案文档必须包含以下章节（按需增删）：

| 章节 | 必须 | 说明 |
|------|------|------|
| 1. 模块定位/概述 | ✅ | 职责、包含/不包含、设计原则 |
| 2. 文件位置 | ✅ | 文件路径 + 划分理由 |
| 3. 依赖关系 | ✅ | 依赖图 + 具体消费的类型/trait |
| 4. 公共 API 设计 | ✅ | trait 定义、结构体、方法签名 |
| 5. 内部实现设计 | ✅ | 算法、数据结构、安全性论证 |
| 6. 实现任务拆分 | ✅ | Wave/Phase 分组，统一粒度 |
| 7. 测试计划 | ✅ | 单元测试 + 集成测试 + 属性测试 + 边界测试 |
| 8. 与其他模块的交互 | ✅ | 接口约定、数据流 |
| 9. 设计决策记录 | 推荐 | ADR 格式 |
| 10. 性能考量 | 推荐 | 复杂度、内存、缓存行为 |
| 11. no_std 兼容性 | 按需 | 条件编译说明 |
| 附录 | 按需 | API 参考、测试清单、运行命令 |

---

## 4. 模块定位与职责边界

### 4.1 职责边界表（必须）

每个模块开篇必须用"包含/不包含"表明确职责边界：

```markdown
| 职责 | 包含 | 不包含 |
|------|------|--------|
| 元素遍历 | 按内存布局顺序遍历 | 具体运算逻辑（由 `ops/` 提供） |
| 轴遍历 | 沿指定轴产子视图 | 子视图的具体操作 |
| 窗口遍历 | 滑动窗口产子视图 | 卷积运算 |
```

### 4.2 设计原则

用简洁的表格列出模块遵循的设计原则：

```markdown
| 原则 | 体现 |
|------|------|
| 步长感知 | 所有迭代器正确处理非连续内存 |
| 零拷贝 | 轴迭代器和窗口迭代器产子视图 |
| trait 一致性 | 统一实现 Iterator/DoubleEndedIterator/ExactSizeIterator |
```

---

## 5. 依赖关系描述

### 5.1 依赖图（ASCII）

使用 ASCII 图展示模块间的依赖方向：

```
src/iter/
├── crate::tensor        # TensorBase<S, D>, 类型别名
├── crate::dimension     # Dimension trait, Ix0~Ix6, IxDyn
├── crate::storage       # Storage, RawStorage trait
└── crate::broadcast     # broadcast_shape() (仅 zip)
```

### 5.2 依赖精确到类型级（必须）

不仅列出依赖哪些模块，还须列出**具体消费了哪些类型/trait**：

```markdown
| 来源模块 | 使用的类型/trait |
|----------|-----------------|
| `tensor` | `TensorBase<S, D>`, `TensorView`, `.shape()`, `.as_ptr()` |
| `dimension` | `Dimension`, `Ix0`~`Ix6`, `IxDyn` |
| `layout` | `LayoutFlags`, `Order` |
```

### 5.3 依赖方向声明

显式声明依赖方向：

> **依赖方向：单向向上。** `iter/` 仅消费 `tensor` 和 `dimension` 等核心模块，不被它们依赖。

---

## 6. 公共 API 设计

### 6.1 完整 trait/结构体定义

API 定义须包含：
- 完整的 trait 签名或结构体定义
- 每个字段的文档注释
- 方法签名（含泛型约束）
- `# Safety` 节（如有 unsafe）

### 6.2 Good / Bad 对比式代码示例（必须）

关键 API 的使用方式须配有 `Good` / `Bad` 对比示例：

```rust
// Good - 使用 ? 和 Result
pub fn reshape<D2>(self, shape: D2) -> Result<Tensor<A, D2>> {
    if self.len() != shape.size() {
        return Err(SenonError::InvalidShape { from: self.len(), to: shape.size() });
    }
    // ...
}

// Bad - 库代码中使用 unwrap
pub fn sum_bad(&self) -> A {
    let first = self.first().unwrap();  // 禁止
    self.iter().fold(*first, |acc, x| acc + x)
}
```

### 6.3 设计决策内联标注

关键设计选择用引用块标注：

```markdown
> **设计决策：** `AxisIter` 的 `Item` 类型为 `TensorView<'a, A, D::Smaller>`。
> 这要求 `Dimension` trait 提供 `type Smaller: Dimension` 关联类型。
```

---

## 7. 内部实现设计

### 7.1 算法描述

核心算法用伪代码或步骤列表描述：

```
increment_index_f(shape, index):
    for i in 0..ndim:
        index[i] += 1
        if index[i] < shape[i]:
            return  // no carry
        index[i] = 0  // carry to next dimension
```

### 7.2 安全性论证

每个 `unsafe` 使用点须有安全性论证：

```markdown
**安全性论证**: `#[repr(C)]` 确保 `usize` 字段连续排列且无 padding，
因此从首字段地址读取 N 个 usize 是安全的。
```

### 7.3 性能考量表

```markdown
| 方面 | 设计决策 |
|------|----------|
| 栈分配 | Ix0~Ix6 全部栈分配，无堆开销 |
| ZST 优化 | Ix0 是零大小类型，编译器完全消除 |
| 内联 | 所有 ndim(), slice() 标注 `#[inline]` |
| 单态化 | Dimension trait 在泛型上下文中单态化，无虚调用开销 |
```

---

## 8. 实现任务拆分

### 8.1 统一任务粒度

每个任务约 **10 分钟**，范围限定在"单个函数或单个 trait impl"。

### 8.2 Wave/Phase 分组

按依赖关系分 Wave 或 Phase，同 Wave 内任务可并行：

```markdown
### Wave 1: 基础设施

- [ ] **T1**: 创建 `src/iter/mod.rs` 骨架
  - 文件: `src/iter/mod.rs`
  - 内容: 模块声明、子模块文件占位、公共导出声明
  - 测试: 编译通过
  - 前置: tensor 模块完成
  - 预计: 5 min
```

### 8.3 任务模板（必须）

每个任务必须包含：

| 字段 | 说明 |
|------|------|
| 任务 ID | T1, T2, ... |
| 描述 | 一句话描述 |
| 文件 | 涉及的文件路径 |
| 内容 | 具体实现内容 |
| 测试 | 对应的测试函数 |
| 前置 | 依赖的任务 |
| 预计 | 预计时间 |

### 8.4 并行执行分组图

用 ASCII 图展示 Wave 间的依赖和并行关系：

```
Wave 1: [W1.1] [W1.2] [W1.3] [W1.6] [W1.8]
           │      │      │      │      │
           └──────┴──────┴──────┴──────┘
                         │
                         ▼
Wave 2: [W2.1] [W2.2] [W2.3] [W2.4] [W2.5]
```

---

## 9. 测试计划

### 9.1 测试分类表

```markdown
| 类型 | 位置 | 目的 |
|------|------|------|
| 单元测试 | `#[cfg(test)] mod tests` | 验证单个函数/方法 |
| 集成测试 | `tests/` | 验证跨模块交互 |
| 边界测试 | 集成测试中标注 | 空数组、单元素、NaN/Inf、非连续 |
| 属性测试 | `tests/property/` | 随机生成验证不变量 |
```

### 9.2 单元测试清单

```markdown
| 测试函数 | 测试内容 | 优先级 |
|----------|----------|--------|
| `test_iter_f_contig` | F-order 连续数组：迭代顺序与物理布局一致 | 高 |
| `test_iter_non_contiguous` | 非连续数组（切片后）：步长跳转正确 | 高 |
| `test_iter_empty` | 空数组：立即返回 None | 中 |
```

### 9.3 边界测试场景表

```markdown
| 场景 | 预期行为 |
|------|----------|
| 空数组 `shape=[0, 3]` | `iter()` 立即结束 |
| 单元素 `shape=[1, 1]` | `iter()` 产出 1 项 |
| 非连续切片 | `iter()` 正确处理步长跳转 |
```

### 9.4 属性测试不变量

```markdown
| 不变量 | 测试方法 |
|--------|----------|
| `iter().count() == tensor.len()` | 随机形状 [0..10, 0..10, 0..10] |
| `axis_iter(Axis(i)).count() == shape[i]` | 随机形状 |
```

---

## 10. 设计决策记录

每个重要设计决策须用四段式 ADR 格式记录：

```markdown
### 决策 N：决策标题

| 属性 | 值 |
|------|-----|
| 决策 | 使用 criterion.rs 作为 benchmark 框架 |
| 理由 | 统计分析（置信区间、异常值检测）；HTML 报告；与 CI 集成成熟 |
| 替代方案 | `#[bench]` nightly — 放弃，需要 nightly 编译器 |
| 替代方案 | divan — 放弃，生态不如 criterion 成熟 |
```

或展开式：

```markdown
### 决策 2：IxDyn 使用 Vec 而非 SmallVec

**背景**：动态维度的内部表示选择。

**选项**：
1. `Vec<usize>`
2. `SmallVec<[usize; 6]>`
3. `ArrayVec<usize, 6>`

**选择**：`Vec<usize>`（当前），SmallVec 作为未来优化

**理由**：
- 保持简单，减少依赖
- 大多数场景下维度数 ≤ 6，堆分配开销可接受
- 未来可通过 feature gate 引入 SmallVec 优化
- API 保持不变
```

---

## 11. 性能描述

### 11.1 具体性能数据

性能对比须给出具体数字，而非仅定性描述：

```markdown
| 场景 | 实现路径 | 每元素开销 | 缓存友好性 |
|------|----------|-----------|-----------|
| 连续 F-order | 指针递增 | 1 次指针加法 | 最优（顺序访问） |
| 非连续（切片） | 步长状态机 | 多次乘加运算 | 较差（跳跃访问） |

**性能数据（参考）**:

| 操作 | 连续数组 | 非连续数组（步长=2） | 性能比 |
|------|----------|---------------------|--------|
| 遍历 1M 元素 | ~1ms | ~3ms | 3x |
| 缓存命中率 | ~95% | ~60% | - |
```

### 11.2 复杂度标注

关键操作标注时间/空间复杂度：

```markdown
- 视图创建：O(1)，不分配内存
- reshape（连续）：O(1)，仅元数据操作
- reshape（非连续）：O(n)，需拷贝数据
```

---

## 12. ASCII 架构图规范

### 12.1 模块依赖图

```
                    ┌──────────┐
                    │  error   │
                    └────┬─────┘
           ┌─────────────┼─────────────┐
     ┌─────┴─────┐  ┌────┴─────┐ ┌─────┴──────┐
     │ dimension │  │ element  │ │   layout   │
     └─────┬─────┘  └────┬─────┘ └─────┬──────┘
           └──────┬──────┴─────────────┘
                  │
            ┌─────┴───────┐
            │   storage   │
            └──────┬──────┘
```

### 12.2 数据流图

```
用户调用 zeros::<f64, Ix2>([3, 4])
    │
    ├── Dimension::ndim()         → 2
    ├── Dimension::slice()        → [3, 4]
    ├── 计算总元素数               → 12
    ├── 计算步长 (F-order)         → [1, 3]
    ├── 对齐分配 12 * 8 = 96 字节  → 64 字节对齐
    └── 返回 TensorBase<Owned<f64>, Ix2>
```

### 12.3 系统架构图

```
┌───────────────────────────────────────────────────────────────┐
│                         迭代器系统架构                          │
│  ┌─────────────────────────────────────────────────────────┐  │
│  │                    Iterator Trait 层                    │  │
│  └─────────────────────────────────────────────────────────┘  │
│                              │                                │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐            │
│  │  Elements   │  │  AxisIter   │  │   Windows   │            │
│  └─────────────┘  └─────────────┘  └─────────────┘            │
└───────────────────────────────────────────────────────────────┘
```

---

## 13. Benchmark 文档规范

### 13.1 四级分类体系

```
Benchmark 分类
├── Micro-benchmarks        # 单操作、单函数级别
├── Kernel benchmarks       # 核心计算内核
├── Workflow benchmarks     # 真实使用模式
└── Comparison benchmarks   # 外部对比（vs ndarray / raw loop / BLAS）
```

### 13.2 参数矩阵标准化

每个 benchmark 须覆盖以下参数组合的**有意义子集**：

#### 输入规模

| 级别 | 1D | 2D | 3D |
|------|-----|-----|-----|
| **Small** | 64 | 8×8 | 4×4×4 |
| **Medium** | 65,536 | 256×256 | 64×32×32 |
| **Large** | 16,777,216 | 4096×4096 | 256×256×256 |

#### 数据类型

| 类型 | 优先级 | 说明 |
|------|--------|------|
| `f64` | **必测** | 科学计算默认精度 |
| `f32` | **必测** | SIMD 向量宽度更大 |
| `Complex<f64>` | **必测** | 复数运算开销验证 |

#### 内存布局

| 布局 | 构造方式 | 验证目标 |
|------|----------|----------|
| F-contiguous | `zeros(shape, Order::F)` | 默认路径性能基线 |
| C-contiguous | `zeros(shape, Order::C)` | C-order 兼容性 |
| Non-contiguous | `tensor.slice(s![.., 0..n-1])` | 非连续路径标量回退惩罚 |

### 13.3 Benchmark 清单表

```markdown
| 组 ID | 操作 | 规模 | 类型 | 布局 | 说明 |
|-------|------|------|------|------|------|
| `elem_add_f64` | `a + b` | S/M/L | f64 | F-contiguous | 连续数组相加 |
| `elem_add_sliced` | `a + b`（b 为切片视图） | M | f64 | Non-contiguous | 非连续惩罚 |
```

### 13.4 CI 三级工作流

| 工作流 | 基准数量 | 预计时间 | 频率 |
|--------|----------|----------|------|
| Smoke Test | 3 个核心文件 × `--quick` | ~5 min | 每次 PR |
| Regression Check | 1-2 个热点基准 | ~10 min | 每次 PR |
| Full Benchmark | 全部文件 × 4 feature 组合 | ~60 min | 每周 |

### 13.5 回归阈值

| 级别 | 阈值 | 动作 |
|------|------|------|
| **警告** | > 5% 变慢 | CI warning（不阻塞合并） |
| **失败** | > 20% 变慢 | CI failure（阻塞合并） |
| **改善** | > 5% 变快 | CI note（记录性能改善） |

---

## 14. 文档编写通则

### 14.1 语言

- 文档主体使用中文
- 代码、注释、变量名、类型名必须使用英文
- 文档中的代码示例须可编译（或标注 `ignore`）

### 14.2 表格优先

能用表格表达的信息优先用表格，避免大段文字描述。

### 14.3 代码示例格式

- 使用 ` ```rust ``` ` 代码块
- 关键示例须配 `// Good` / `// Bad` 标注
- 涉及 unsafe 的代码须配 `// SAFETY:` 注释

### 14.4 交叉引用

- 引用其他文档时用反引号标注：`参见 01-architecture-overview.md §3`
- 引用需求文档时用章节号：`需求说明书 §7.1`

### 14.5 版本历史

- 文档末尾在页脚之前加入版本历史
- 不需要说明列
- 版本号升序排列

```markdown
## 版本历史

| 版本 | 日期 |
|------|------|
| 1.0.0 | 2026-04-07 |
| 1.0.1 | 2026-04-08 |
| 1.0.2 | 2026-04-08 |
```

### 14.6 页脚

每个文档末尾须包含：

```markdown
---

*本文档由 <项目名> 维护。如有问题请提交 Issue 或 PR。*
```

---

## 15. 文档质量检查清单

文档完成后，用以下清单自检：

| 检查项 | 通过 |
|--------|------|
| 元信息头完整（编号、模块路径、阶段、前置文档） | □ |
| 职责边界表（包含/不包含）已填写 | □ |
| 依赖关系精确到类型级 | □ |
| 公共 API 定义完整（trait/结构体/方法签名） | □ |
| 关键 API 有 Good/Bad 对比示例 | □ |
| 设计决策用 ADR 格式记录 | □ |
| 实现任务按 Wave 分组，粒度统一（~10 min） | □ |
| 每个任务包含：文件、内容、测试、前置、预计 | □ |
| 测试计划覆盖：单元 + 集成 + 边界 + 属性 | □ |
| 测试用例以表格列出（名称/内容/优先级） | □ |
| ASCII 依赖图清晰 | □ |
| 性能数据具体化（有数字，非仅定性描述） | □ |
| no_std 兼容性说明（如适用） | □ |
| 交叉引用格式正确 | □ |
| 示例代码与算法正确无误 | □ |
| 示例代码与算法采用英文编写 | □ |
| 版本历史完整 | □ |

