...
pub enum TypedViewRejection {
    ...
    /// `count.checked_mul(size_of::<T>())` overflowed `usize`.
...
### Step 4: 为 FfiErrorCategory 和 ConversionFailureReason 实现 Display
...
> **注意**: `FmtShape` 辅助结构体由 W2T3 定义，上述 Display 实现引用了该类型。W2T3 的 Step 1 定义了 `FmtShape`，此依赖关系不影响编译顺序（同一文件内，定义顺序可由开发者安排）。
...
mod tests {
    ...
    fn test_ffi_error_category_variants_construct() {
        ...
        assert!(format!("{}", e).contains("invalid rank"));
...
| `FfiErrorCategory` + `ConversionFailureReason` 实现 `Display` | `26-error §5.1` 内嵌 Display impl，提供人类可读的错误分类名 |
...
- `FfiErrorCategory::Display` 引用 `FmtShape`（W2T3 定义），代码需安排在 `FmtShape` 定义之后
