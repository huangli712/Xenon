...
| W3T2 | `src/dimension/mod.rs` | Dimension trait definition (all method signatures, MAX_DIMENSION constant) | W3T1 | 02-dimension §5, §6, §7, §8 |
...
| Task | File | Goal | Dependencies | Design Docs |
|------|------|------|-------------|-------------|
| W5T1 | `src/complex/mod.rs` | Complex\<T\> struct (repr(C)) + new() constructor | W1T3 | 04-complex §5, §6, §7, §8 |
| W5T2 | `src/complex/mod.rs` | ComplexFloat sealed trait + f32/f64 impls | W5T1, W3T15 | 04-complex §5, §6, §7, §8 |
| W5T3 | `src/complex/mod.rs` | const size/align assertions for FFI layout guarantees | W5T1 | 04-complex §5, §6, §7, §8 |
| W5T4 | `src/complex/mod.rs` | re() / im() accessors | W5T1 | 04-complex §5, §6, §7, §8 |
| W5T5 | `src/complex/mod.rs` | from_imag(), conj(), and From\<T\> constructors | W5T1, W5T4 | 04-complex §5, §6, §7, §8 |
| W5T6 | `src/complex/mod.rs` | is_real() / is_imaginary() predicates | W5T1 | 04-complex §5, §6, §7, §8 |
| W5T7 | `src/complex/mod.rs` | PartialEq (NaN!=NaN) + Display (a+bj format) impls | W5T1 | 04-complex §5, §6, §7, §8 |
| W5T8 | `src/complex/ops.rs` | Complex Add operator | W5T1 | 04-complex §5, §6, §7, §8 |
| W5T9 | `src/complex/ops.rs` | Complex Sub operator | W5T8 | 04-complex §5, §6, §7, §8 |
| W5T10 | `src/complex/ops.rs` | Complex Mul operator | W5T1 | 04-complex §5, §6, §7, §8 |
| W5T11 | `src/complex/ops.rs` | Complex Div operator | W5T10 | 04-complex §5, §6, §7, §8 |
| W5T12 | `src/complex/ops.rs` | Complex Neg operator | W5T1 | 04-complex §5, §6, §7, §8 |
| W5T13 | `src/complex/ops.rs` | Tighten real-complex mixed arithmetic boundary (Complex op Complex only, explicit conversion) | W5T8–W5T12 | 04-complex §5, §6, §7, §8 |
| W5T14 | `src/complex/mod.rs` | Math methods: norm (hypot), norm_sqr | W5T1 | 04-complex §5, §6, §7, §8 |
...
| W6T5 | `src/layout/flags.rs` | LayoutFlags(u8) bitflags: F_CONTIGUOUS, ALIGNED, HAS_ZERO_STRIDE + query/set methods | W6T2 | 06-layout §5, §6, §7, §8 |
| W6T6 | `src/layout/strides.rs` | compute_f_strides\<D\>: F-order stride computation with overflow check returning Result | W6T3, W3T2 | 06-layout §5, §6, §7, §8 |
| W6T7 | `src/layout/contiguous.rs` | is_f_contiguous\<D\>: F-order contiguity detection | W6T4, W3T2 | 06-layout §5, §6, §7, §8 |
| W6T8 | `src/layout/strides.rs` | has_zero_stride: raw zero-stride detector (flag assignment checks product(shape) > 0) | W6T3, W3T2 | 06-layout §5, §6, §7, §8 |
| W6T9 | `src/layout/strides.rs` | is_aligned_to / is_aligned alignment check functions | W6T3 | 06-layout §5, §6, §7, §8 |
| W6T10 | `src/layout/` (`#[cfg(test)] mod tests`), `tests/test_tensor.rs`, `tests/test_shape.rs`, `tests/test_index.rs`, `tests/test_ffi.rs`, `tests/test_simd.rs` | Layout integration tests | W6T6–W6T9 | 06-layout §5, §6, §7, §8 |
...
| W7T12 | `src/storage/owned.rs` | Owned\<A\> into_shared + unsafe impl Send + unsafe impl Sync | W7T8, W7T16 | 05-storage §5, §6, §7, §8 |
| W7T13 | `src/storage/owned.rs` | Owned\<A\> From + Default impls | W7T8 | 05-storage §5, §6, §7, §8 |
...
| W7T17 | `src/storage/arc.rs` | ArcRepr\<A\> Send/Sync + Default + From impls | W7T16 | 05-storage §5, §6, §7, §8 |
...
| W10T5 | `src/dispatch.rs` | Threshold config: AtomicUsize storage, set/reset_parallel_threshold, Relaxed ordering | W10T4 | 30-dispatch §5, §6, §7, §8 |
...
| W15T7 | `src/parallel/checked.rs` | Error + panic propagation: XenonError passthrough, panic not swallowed | W15T2, W15T4, W15T5 | 09-parallel §5, §6, §7, §8 |
| W15T8 | `src/parallel/` all files, `tests/test_parallel.rs` | Feature gate + config matrix tests: default off, --features parallel build, single/multi-thread branch validation | W15T1–W15T7 | 09-parallel §5, §6, §7, §8 |
...
| W16T6 | `src/math/binary.rs` | Arithmetic ops: add, sub, mul, div (scalar variants) via shared binary skeleton | W16T2 | 11-math §5, §6, §7, §8 |
...
| W18T2 | `src/reduction/sum.rs` | sum(): full traversal, integer checked arithmetic, empty array zero semantics | W18T1 | 13-reduction §5, §6, §7, §8 |
| W18T3 | `src/reduction/sum.rs` | sum_axis(): axis validation, output shape reduction, per-axis slot accumulation | W18T2 | 13-reduction §5, §6, §7, §8 |
| W18T4 | `src/reduction/sum.rs` | sum_axis_keepdims(): reuse per-axis logic, preserve reduced axis as length 1 | W18T3 | 13-reduction §5, §6, §7, §8 |
| W18T5 | `src/reduction/sum.rs`, `src/simd/`, `src/parallel/` | SIMD / parallel dispatch guards: wire dispatch results, prevent routing semantically invalid inputs | W18T4, W14, W15 | 13-reduction §5, §6, §7, §8 |
| W18T6 | `src/reduction/sum.rs`, `tests/test_reduction.rs` | Error semantic convergence: axis OOB → InvalidAxis, integer overflow → panic | W18T3–W18T5 | 13-reduction §5, §6, §7, §8 |
...
| Task | File | Goal | Dependencies | Design Docs |
|------|------|------|-------------|-------------|
| W20T1 | `src/shape/mod.rs` | Module skeleton: sub-module declarations, public exports | None | 16-shape §5, §6, §7, §8 |
