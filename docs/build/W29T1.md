...
/// The public surface follows design §5.2 L358-359 `impl RealScalar`
/// signatures. `RealScalar` (03-element.md §5.3) does not expose `to_bits`
/// itself, so implementations dispatch through a crate-private extension
/// trait `RealScalarBits` providing `to_bits` for `f32` and `f64` only.
