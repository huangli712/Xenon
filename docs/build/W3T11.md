...
| Task | W3T11 — IxDyn dynamic dimension with Dimension impl + constructors |
...
依据 `02-dimension.md` §5.5 (lines 426-452) 为 `IxDyn` 实现 `Dimension`，覆盖 trait 全部方法：`NDIM`, `ndim`, `slice`, `checked_size`, `checked`, `try_from_slice`。`axis` 使用 trait 默认实现。不在此处实现 `into_dyn`/`try_from_dyn`——它们是 inherent 方法，由 W3T12 添加（见 §5.1 设计决策）。`checked_size` 必须逐轴 checked_mul 并在溢出时填入 `offending_dim: Some(axis)`（§5.5 lines 433-444）。
...
3. 依据 §5.1 设计决策，`into_dyn` / `try_from_dyn` 不在 `Dimension` trait 中；它们由 W3T12 集中实现为 inherent 方法（IxDyn 的 identity conversion）。
