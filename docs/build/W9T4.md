...
- 确认 W9T2 已提供 `Workspace` 结构体、原子字段 `borrow_state` / `split_count` 与 `BORROW_*` 常量；本任务通过 `pub(crate)` 字段直接读取这些原子。
...
- Workspace 和所有 guard 保持 `!Send + !Sync`（由 W9T2 PhantomData 强制；W9T7 编译期负向断言）。
