...
| `aliases.rs` 内 `pub use super::TensorBase;`（隐式可见） + 直接引用 `super::TensorBase` 在所有别名定义中 | 避免循环 use；`TensorBase` 由 W8T2 在 `mod.rs` 定义 |

## 注意事项

- 所有别名通过 `pub type` 定义在 `aliases.rs`，类型参数顺序统一为 `<A, D>`（不带默认值）。
- `mod.rs` 在 W8T3 完成时追加 `pub use aliases::*;` —— 该 re-export 由 W8T3 自身在 `aliases.rs` 末尾通过 `pub use ...` 形式不需要（`pub type` 已经是 pub 可见）；若需要在 `tensor::*` 命名空间统一导出，可在 `mod.rs` 加 `pub use self::aliases::*;`。
- 别名仅做类型替换，不引入新方法。
