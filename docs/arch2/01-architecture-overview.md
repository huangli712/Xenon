...
> 本文档定义 Xenon 的整体架构，包括目录结构、模块层次、依赖关系和实施阶段。

---

## 1. 项目概览

Xenon 是一个 Rust 多维数组（张量）库，定位为科学计算的数值基础设施。核心抽象为双参数泛型 `TensorBase<S, D>`，其中 S 决定存储模式（所有权/视图），D 决定维度类型（静态/动态）。
...
`src/lib.rs` 负责 re-export 所有公共类型，用户只需 `use xenon::...`：
