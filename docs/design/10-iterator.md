...
| 职责         | 包含                                                                         |
| ------------ | ---------------------------------------------------------------------------- |
| 元素遍历     | 按元素遍历（逻辑 F-order 索引顺序）                                          |
| 轴遍历       | 沿指定轴产生子张量视图                                                       |
| 索引遍历     | 带多维索引的元素遍历                                                         |
| 内部能力提示 | 各操作模块可直接实现自身所需的内部迭代分发；`Windows` 可作为后续版本议题保留 |
| 并行迭代     | —                                                                            |

| 职责         | 不包含                                                                      |
| ------------ | --------------------------------------------------------------------------- |
| 元素遍历     | 具体运算逻辑（参见 `11-math.md §1`）                                        |
| 轴遍历       | 子视图的具体操作                                                            |
| 索引遍历     | 索引赋值操作                                                                |
| 内部能力提示 | 不单独设计统一的多输入 lock-step 中间抽象；`Windows` 不纳入当前版本实现范围 |
...
| 类型     | 内容                                                                                                                       |
| -------- | -------------------------------------------------------------------------------------------------------------------------- |
| 需求映射 | 需求说明书 §6 - §8、§10、§11、§16 - §18、§21、§27、§28                                                                     |
...
for i in 0..tensor.shape()[0] {
    for j in 0..tensor.shape()[1] {
        let _ = tensor.try_at((i, j))?;  // not recommended; see 17-indexing v3.0.1 决策 7
    }
}
