...
| `simd_auto_path_switch`    | SIMD 自动路径选择       | S/M/L | f32/f64        | F-contiguous   | 验证实现根据数据规模自动选择 SIMD 或标量路径的切换行为                           |
...
- [ ] **T10**: 实现 `benches/simd_comparison.rs`
  ...
  - 内容: add/sum/dot 以及 `simd_auto_path_switch` 在 `--features simd` 开/关时的性能对比
...
- [ ] **T11**: 实现 `benches/parallel_comparison.rs`
  ...
  - 内容: sum/add/dot 在 `--features parallel` 开/关时的性能对比，并补 `auto_threshold_switch` / `nested_parallel_fallback`
  - 测试: 分别以两种 feature 配置运行，对比结果
  - 前置: T3, T4
  - 预计: 10 min
