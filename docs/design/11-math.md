...
| 场景 | 错误 |
| ---- | ---- |
| 二元运算广播失败 | 返回 `XenonError::BroadcastError`；其中 `input_shape` 表示原始输入 shape（例如 lhs），`target_shape` 表示尝试广播到的目标 shape。 |
...
| 广播形状不兼容        | 返回 `XenonError::BroadcastError`；其中 `input_shape` 表示原始输入 shape（例如 lhs），`target_shape` 表示尝试广播到的目标 shape |
...
| 主题 | 内容 |
| ---- | ---- |
| Recoverable error | 广播不兼容时返回 `XenonError::BroadcastError`；其中 `input_shape` 表示原始输入 shape（例如 lhs），`target_shape` 表示尝试广播到的目标 shape。参数不满足公开前提时返回 `XenonError::InvalidArgument { operation: Cow<'static, str>, argument: Cow<'static, str>, expected: Cow<'static, str>, actual: Cow<'static, str>, axis: Option<usize>, shape: Option<Vec<usize>> }`。 |
