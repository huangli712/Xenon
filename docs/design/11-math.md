...
| 场景 | 错误 |
| ---- | ---- |
| 二元运算广播失败 | 返回 `XenonError::BroadcastError { lhs_shape, rhs_shape, attempted_result_shape }`，分别记录左右输入 shape 与本次尝试构造的广播结果 shape。 |
...
> **数学函数精度约束：** `sin` / `sqrt` / `exp` / `ln` / `floor` / `ceil` 使用 Rust 提供的数学能力，不引入外部数学 crate；实现必须在文档与测试中声明其采用的 ULP 误差边界。当前版本将公开容差固定为下表：

| 函数 | 容差(ULP) | 说明 |
| ---- | --------- | ---- |
| `sin` / `cos` | ≤ 4 ULP | 依赖 `libm` / 平台实现 |
| `sqrt` | ≤ 1 ULP | 硬件指令通常精确 |
| `exp` / `ln` | ≤ 4 ULP | 依赖 `libm` / 平台实现 |
| `floor` / `ceil` | 0 ULP | 精确操作 |
...
| 操作类别 | SIMD 状态 | 并行状态 |
| -------- | --------- | -------- |
| 算术(`+ - * /`) | 已覆盖（`f32` / `f64` / `Complex`） | 已覆盖 |
| 一元（`neg` / `abs` / `square`） | 部分覆盖 | 规划中 |
| 比较（`eq` / `ne` / `lt` / `gt`） | 规划中 | 规划中 |
| 数学（`sin` / `sqrt` / `exp` / `ln` / `floor` / `ceil`） | 标量回退 | 标量回退 |
| 复数（`modulus` / `conjugate`） | 标量回退 | 标量回退 |
| 逻辑（`not`） | 标量回退 | 标量回退 |
...
| 广播形状不兼容        | 返回 `XenonError::BroadcastError { lhs_shape, rhs_shape, attempted_result_shape }` |
...
| 主题 | 内容 |
| ---- | ---- |
| Recoverable error | 广播不兼容时返回 `XenonError::BroadcastError { lhs_shape, rhs_shape, attempted_result_shape }`。参数不满足公开前提时返回 `XenonError::InvalidArgument { operation: Cow<'static, str>, argument: Cow<'static, str>, expected: Cow<'static, str>, actual: Cow<'static, str>, axis: Option<usize>, shape: Option<Vec<usize>> }`。 |
...
| 容差边界 | `floor` / `ceil` 的 SIMD/并行路径结果必须与标量路径逐元素完全一致，不允许容差；`sin` / `sqrt` / `exp` / `ln` 的容差采用本文档 §5.5 中的 ULP 表。复数结果按实部、虚部分量分别应用对应实数容差规则；仅在可证明语义等价或满足文档化容差时启用 SIMD，否则回退标量路径。 |
