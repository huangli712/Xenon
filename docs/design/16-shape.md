...
>
> **共享语义补充：** 若源张量为 `ArcRepr`（共享只读），转置后 `storage_kind()` 返回 `StorageKind::View` 而非 `Shared`。这是有意设计：转置结果的生命周期绑定到调用时借用，而不是源张量的共享引用计数。即使源张量为 `ArcRepr`（Shared），`transpose()` 结果也只返回借用视图（`ViewRepr`），`storage_kind()` 返回 `View`。这是允许的收窄：`require.md` §17 只要求结果落在只读引用或共享只读引用范围内。借用视图满足只读引用约束。对后续广播、格式化、线程共享的影响：转置结果的生命周期绑定到原始张量的借用期。
