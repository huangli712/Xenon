...
> **范围说明：** 当前版本的 `transpose()` 定义为全轴顺序反转（reverse axis order），等价于矩阵转置。一般化的任意轴置换不在当前版本范围内。
>
...
>
> **共享语义补充：** 若源张量为 `ArcRepr`（共享只读），转置后 `storage_kind()` 返回 `StorageKind::View` 而非 `Shared`。这是有意设计：转置结果的生命周期绑定到调用时借用，而不是源张量的共享引用计数。
