...
> **共享可写边界说明：** ArcRepr 相关的唯一化（uniquify）后恢复独占写能力仅是内部实现机制。这不构成共享可写存储模式。当前版本不提供共享可写存储模式（参见 `require.md §6.1`）。
...
- 导出描述符本身是否可 `Send` / `Sync`，取决于其是否仅承载只读元数据，还是承载可写裸指针与独占语义；文档与测试须显式区分这两类导出。
...
| 描述符类型 | `Send` | `Sync` | 说明 |
|-----------|--------|--------|------|
| `TensorExport<'a, A>` | 随 `A: Sync` | 否（含借用） | 只读导出，可跨线程发送但不可共享 |
| `TensorExportMut<'a, A>` | 随 `A: Send` | 否（含独占借用） | 可写导出，仅可跨线程发送 |

须在 `23-ffi.md` 中同步设计并在测试计划中覆盖编译期 `Send/Sync` 检查。
