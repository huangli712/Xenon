...
### 4.5 ViewMutRepr<'a, A> 的 Send（不实现 Sync）
...
let a = Tensor1::from_shape_vec([3], vec![1.0, 2.0, 3.0]).unwrap();
let b = a.view().broadcast_to([3, 3]).unwrap();  // broadcast result: TensorView
...
| 测试文件 | 测试内容 |
|----------|----------|
| `tests/thread_safety.rs` | `Owned` / `View` / `ViewMut` / `ArcTensor` 与 `parallel`、`workspace`、跨线程传递场景的端到端协同验证 |
...
Workspace 模块的 `split_at`/`split_at_mut` 使用原子引用计数（`AtomicUsize`）管理分割守卫的生命周期。安全性保证：

- `split_at()` 设置 `split_count = 2`（二分产生 2 个子空间）
- `split_at_mut()` 原子递增 `split_count`（递归分割产生净 +1 个活跃守卫）
