...
/// # Visibility Note
///
/// `Float` is `pub(crate)` — only `f32` and `f64` implement it.
/// `Complex<T>` full functionality (arithmetic, math) is only available
/// when `T = f32` or `T = f64`. Third-party float types are not supported.
...
> **中间溢出风险：** 当 `self.re` 为大负数时，`r - self.re`（即 `norm() + |re|`）可能导致中间结果溢出为 `+inf`。虽然后续 `sqrt(+inf)` 仍为 `+inf`，结果并非错误，但精度会有损失。对模长接近 `f32::MAX` / `f64::MAX` 且实部为大负数的复数，建议使用基于 `arg()` 的替代算法：`sqrt(r) * (cos(arg/2) + i*sin(arg/2))`。

### 4.6 算术运算实现

> **设计说明：** `Complex<T>` 能否满足下游 `Numeric` trait 的约束，取决于 `Float` trait 是否包含了所有必要的运算符 trait（`Add`、`Sub`、`Mul`、`Div`、`Neg`）。由于 `Float` 的 supertrait 列表已包含这些运算符，`T: Float` 约束自动保证 `Complex<T>` 可实现全部算术运算。
...
> **约束说明：** `PartialEq` 仅在 `T: Float`（即 `T = f32` 或 `T = f64`）时可用。这是因为 `Float` trait 的 supertrait 包含 `PartialEq`，而 `Complex<T>` 的 `PartialEq` impl 要求 `T: Float`。
...
impl<T: Float + core::fmt::Display> core::fmt::Display for Complex<T> {
    ...
    ///
    /// -0.0 handling: `im == T::zero()` catches both `+0.0` and `-0.0`,
    /// so `-0.0` imaginary part is formatted as pure real (e.g., `"3"` for
    /// `Complex::new(3.0, -0.0)`). This is intentional — IEEE 754 defines
    /// `-0.0 == +0.0` as true.
