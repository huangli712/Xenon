...
> **依赖方向：单向向下。** storage 模块不依赖 layout；实现层主要使用标准库及既有 `core` / `alloc` 能力。`storage/` 不直接依赖 `element` 模块；元素类型约束通过 `TensorBase` 的泛型参数间接体现（`Storage::Elem` 关联类型）。`tensor/`（参见 `07-tensor.md` §5）和 `iter/`（参见 `10-iterator.md` §4）模块消费 storage 的 trait 和类型。
...
| `Owned<A>`           | `ArcRepr<A>`         | O(1)        | 复用同一对齐缓冲并建立共享拥有           |
...
| -------------------- | --------------------------- | ----------------------- | --------------------------- | ----------------------------- |
| `Owned<A>`           | —                           | 零拷贝                  | 零拷贝                      | 须分配                        |
| `ViewRepr<'_, A>`    | 须分配                      | —                       | 不可转换                    | 须分配                        |
| `ViewMutRepr<'_, A>` | 须分配                      | 零拷贝                  | —                           | 须分配                        |
| `ArcRepr<A>`         | 须分配                      | 零拷贝                  | 须分配                      | —                             |

> **补充说明：** 上表按四种具体存储表示细化 §6.2 的抽象规则：零拷贝表示仅重借用或降级访问权限；须分配表示需要克隆数据或触发 CoW 生成独立缓冲；不可转换表示违反 Rust 的可变性/独占借用约束，例如 `ViewRepr<'_, A> → ViewMutRepr<'_, A>`。
...
/// same backing-allocation invariants as `Owned<A>`. Copy-on-write (CoW) happens
/// at the aligned buffer level via `make_mut()`.
#[derive(Debug)]
...
> **设计决策：** `ArcRepr<A>` 不存储独立的 `offset` 字段。偏移量与切片范围由外层 `TensorBase` 元数据管理（见 `07-tensor.md §5.1`），而 `ArcRepr` 只表示共享拥有的底层连续缓冲区。`make_mut()` 仅针对整个底层缓冲区执行写时复制，用于产生独占 owned 数据，不负责解释张量视图的 offset/shape/strides，也不把 `ArcRepr` 提升为共享可写存储模式。
