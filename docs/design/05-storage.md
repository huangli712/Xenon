...
| `ArcRepr<A>` | ✅ (共享) | ✅ | make_mut() | 浅拷贝 (引用计数+1) | 共享对齐缓冲，写时按需深拷贝 |
...
> **设计决策：** `ArcRepr<A>` 不存储独立的 `offset` 字段。偏移量与切片范围由外层 `TensorBase` 元数据管理（见 `07-tensor.md §4.1`），而 `ArcRepr` 只表示共享拥有的底层连续缓冲区。`make_mut()` 仅针对整个底层缓冲区执行写时复制，不负责解释张量视图的 offset/shape/strides。
...
| `ArcRepr<A>` | ✅* | 使用 `alloc::sync::Arc`，目标需支持原子引用计数 |
| `AlignedAlloc` | ✅ | 使用 `alloc::alloc` |

> `✅*` 表示需要目标平台支持 `Arc` 所需的原子操作；若目标不支持原子引用计数，则 `ArcRepr` 不可用，但 `Owned` / `ViewRepr` / `ViewMutRepr` 仍可保持可用。
