...
| **错误语义集中裁决** | 可恢复错误统一通过 `Result` 报告；索引语法 `tensor[[...]]` 作为语言级索引接口保留 panic，运算符与方法型 API 使用同一可恢复错误模型 |
...
| MSRV | Rust 1.85+ |
| License | MIT |
| no_std | 支持 `no_std`（依赖 alloc）；其中 workspace / Arc 等子系统在 no_std 下额外要求目标平台提供原子能力，`std` 为默认 feature |
...
| no_std 数学函数 | `RealScalar` / `ComplexScalar` 的超越函数与数学函数仅在 `std` feature 下提供实现；`no_std + alloc` 环境保留核心张量/存储/视图能力 |
...
│   │   └── primitives.rs      # 基础类型 impl（f32, f64, i32, i64, bool）
│   │
│   ├── complex/               # 自定义复数类型
...
│   │   ├── fill.rs            # fill（原地填充）
│   │   └── contiguous.rs      # to_contiguous（连续性保证的公共入口）
│   │
│   ├── set/                   # 集合操作
...
│   │   └── contiguous.rs      # contiguous helper（仅供 convert/util 内部复用）
│   │
│   ├── format/                # 格式化输出
...
| **L2** | workspace | core, alloc, atomics（独立于核心类型系统，可被上游库直接使用） | `24-workspace.md` |
...
| 属性 | 值 |
|------|-----|
| 决策 | 可恢复错误统一通过 `Result` 暴露；索引语法 `tensor[[...]]` 作为语言级索引接口保留 panic，运算符语法与方法型 API 共享同一错误语义 |
