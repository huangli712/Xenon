...
> 前置文档: `00-coding.md`
> 需求参考: `需求说明书 §1`, `需求说明书 §2`, `需求说明书 §3`, `需求说明书 §4`, `需求说明书 §5`, `需求说明书 §6`, `需求说明书 §7`, `需求说明书 §8`, `需求说明书 §9`, `需求说明书 §10`, `需求说明书 §11`, `需求说明书 §12`, `需求说明书 §13`, `需求说明书 §14`, `需求说明书 §15`, `需求说明书 §16`, `需求说明书 §17`, `需求说明书 §18`, `需求说明书 §19`, `需求说明书 §20`, `需求说明书 §21`, `需求说明书 §22`, `需求说明书 §23`, `需求说明书 §24`, `需求说明书 §25`, `需求说明书 §26`, `需求说明书 §27`, `需求说明书 §28`
> 范围声明: 范围内
...
> **对齐说明：** `需求说明书 §10` 仅要求实现具备对齐分配/对齐访问支持，不规定统一的全局对齐字节数；64 字节是当前实现建议，后续可在不破坏契约的前提下调整。
...
| 类型     | 内容                                                                 |
| -------- | -------------------------------------------------------------------- |
| 需求映射 | `需求说明书 §1`, `需求说明书 §2`, `需求说明书 §3`, `需求说明书 §4`, `需求说明书 §5`, `需求说明书 §6`, `需求说明书 §7`, `需求说明书 §8`, `需求说明书 §9`, `需求说明书 §10`, `需求说明书 §11`, `需求说明书 §12`, `需求说明书 §13`, `需求说明书 §14`, `需求说明书 §15`, `需求说明书 §16`, `需求说明书 §17`, `需求说明书 §18`, `需求说明书 §19`, `需求说明书 §20`, `需求说明书 §21`, `需求说明书 §22`, `需求说明书 §23`, `需求说明书 §24`, `需求说明书 §25`, `需求说明书 §26`, `需求说明书 §27`, `需求说明书 §28` |
| 范围内   | 总体分层、模块边界、feature gate、依赖约束、错误语义与质量边界       |
| 范围外   | 单模块内部算法、实现细节、额外平台与超范围能力扩展                   |
| 非目标   | 通过总体架构文档新增需求未授权的 API、依赖或多 crate 拆分            |

> **验证责任补充说明：** 架构文档对 `需求说明书 §28.3` / `需求说明书 §28.4` / `需求说明书 §28.5` 的职责是定义跨模块一致的验证边界：§28.3 负责数值/错误/路径一致性总原则，§28.4 负责大型输入/高维/极值等系统级边界测试占位，§28.5 仅负责并行/SIMD 路径一致性与 `usize` / 其他无符号整数约束验证。FFI 边界约束归口 `需求说明书 §25`；本文后续给出的发布前核对矩阵仅作为非规范性工程治理清单，不替代 `需求说明书 §28` 的强制要求。
...
| `dispatch.rs`  | 私有内部执行路径裁决层（`ExecPath`、`ParallelGuard`、并行阈值），不作为公开模块导出 |
...
| `parallel/`    | 并行后端：位于 **L5** 的独立 backend 模块，承载纯并行执行入口（par_map/par_zip_map/par_sum/par_dot；阈值裁决由 `dispatch.rs` 统一处理）与内部并行迭代 helper，不含串行回退；所有公开并行加速均由 L6 语义模块经 `dispatch.rs` 内部裁决后按需委托 |
...
| `convert/`     | 类型转换（core surface = `cast` / `CastTo`；`to_owned()` / `into_owned()` 作为同模块便利 API，共享同一转换矩阵基础设施） |
...
| **L5** | broadcast, iter, ffi, simd, dispatch, parallel             | tensor（simd 额外依赖 layout/element；parallel 额外依赖 `rayon` feature） | `15-broadcast.md`、`10-iterator.md`、`23-ffi.md`、`08-simd.md`、`09-parallel.md`、`01-architecture.md`                           |
| **L6** | math, overload, set, matrix, reduction, shape, index, util | tensor, broadcast，以及按需调用 `dispatch` 做路径选择并委托 `simd` / `parallel` 后端或模块内串行实现 | `11-math.md`、`12-matrix.md`、`13-reduction.md`、`14-set.md`、`16-shape.md`、`17-indexing.md`、`19-overload.md`、`20-utility.md` |
...
L4:  tensor
      │
L5:  iter    broadcast    ffi    simd    dispatch    parallel
      │         │           │      │        │           │
      └─────────┴───────────┴──────┴────────┴───────────┘
                           │
...
本文档不直接定义错误类型，但要求所有架构层级、模块边界与执行路径统一遵循单一 `XenonError` 公开错误模型；架构层只裁决错误入口应单一、路径语义应一致，不在此重复定义完整错误枚举。所有可恢复错误直接以 `XenonError` 结构化变体返回，不使用模块内部错误类型；规范错误模型的 canonical source 以 `26-error.md` 为准。对于 FFI 场景，公开 Rust 入口包含结构化导出 `export()` / `export_mut()` 与 checked 查询 `try_offset_of()` / `try_ptr_at()`，并统一通过 checked arithmetic 计算偏移与指针。
...
| 需求说明书 §28.4 占位：large-tensor | 大张量下架构分层与执行路径仍不破坏错误/性能边界 |
| 需求说明书 §28.4 占位：high-dim | 高维输入下模块边界、广播/转置/索引协作仍保持成立 |
| 需求说明书 §28.4 占位：extreme-value | 极值/NaN/Inf 输入下跨模块语义保持一致 |
