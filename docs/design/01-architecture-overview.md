...
| 外部依赖 | 仅 rayon（可选并行）+ pulp（可选 SIMD）+ libm（no_std 环境下数学函数必需，通过 `libm` feature 启用） |
...
# Math functions in no_std (sin, cos, exp, ln, etc.)
# Automatically enabled when std is OFF; redundant when std is ON.
libm = ["dep:libm"]
