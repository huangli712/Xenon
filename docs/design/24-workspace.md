...
| -------- | ---- |
| 需求映射 | 需求说明书 §26 |
| 范围内   | 对齐分配、借用守卫、`split_at` / `split_at_mut`、`ensure_capacity` 单向增长，以及 `MaybeUninit` scratch 语义。 |
...
`WorkspaceError` 保留为 workspace 模块内部与模块级 API 的错误分类，用于表达分配失败、借用冲突、分割越界等局部语义；当错误穿过 Xenon 的公开 API 边界时，须包装为 `XenonError::Workspace(WorkspaceError)`。因此本文档中的 `new()`、`borrow()`、`borrow_mut()`、`split_at()`、`ensure_capacity()` 等公开入口在最终对外签名上统一使用 `Result<_, XenonError>`，而不是直接暴露 `WorkspaceError`。
...
impl Workspace {
    ...
    /// ```
    pub fn new(capacity: usize, alignment: usize) -> Result<Self, XenonError> {
    ...
    pub fn with_default_capacity() -> Result<Self, XenonError> {
...
impl Workspace {
    ...
    ///
    /// `borrow()`/`borrow_mut()` take `&self` rather than `&mut self` because exclusivity is enforced at runtime by the internal `AtomicU8` state machine instead of Rust's compile-time borrow checker. The caller must ensure that multiple active borrow guards are not held on the same `Workspace` at the same time.
    pub fn borrow(&self) -> Result<WorkspaceBorrow<'_>, XenonError> {
    ...
    ///
    /// `XenonError::Workspace(WorkspaceError::AlreadyBorrowed)`: Workspace is already borrowed.
    pub fn borrow_mut(&self) -> Result<WorkspaceBorrowMut<'_>, XenonError> {
        let prev = self.borrow_state.compare_exchange(
            Self::BORROW_NONE,
...
impl Workspace {
    ...
    pub fn split_at(
        &self,
        mid: usize,
    ) -> Result<(SplitBorrowMut<'_>, SplitBorrowMut<'_>), XenonError> {
...
| typed borrow 拒绝 ZST 与不对齐区域 | 编译期 / 运行时断言测试。 |
...
| 主题 | 内容 |
| ---- | ---- |
| Recoverable error | `new()` / `ensure_capacity()` / `borrow*()` / `split_at*()` 失败时返回 `WorkspaceError`（经公开边界包装为 `XenonError::Workspace`），携带容量、对齐、分割点或借用状态上下文。 |
| Panic | typed `assume_init_*` / typed borrow helper 的对齐、长度溢出与 ZST 断言违反会 panic；unsafe 初始化前提违反属于 UB。 |
