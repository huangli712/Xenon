...
| §27 错误处理 | 公开入口对长度、对齐、ZST、越界和扩容溢出统一返回 `Result<_, XenonError::Workspace(...)>`。 |
...
`WorkspaceError` 保留为 workspace 模块内部与模块级 API 的错误分类，用于表达分配失败、借用冲突、分割越界等局部语义；当错误穿过 Xenon 的公开 API 边界时，须包装为 `XenonError::Workspace(WorkspaceError)`。因此本文档中的 `new()`、`borrow()`、`borrow_mut()`、`split_at_mut()`、`ensure_capacity()` 等公开入口在最终对外签名上统一使用 `Result<_, XenonError>`，而不是直接暴露 `WorkspaceError`。
...
impl<'a> WorkspaceBorrowMut<'a> {
    ...
    pub unsafe fn as_maybe_uninit_typed_slice<T>(
        &mut self,
        count: usize,
    ) -> Result<&mut [core::mem::MaybeUninit<T>], XenonError> {
...
let _again = ws.split_at_mut(128)?;  // Returns `AlreadyBorrowed` at runtime
```
