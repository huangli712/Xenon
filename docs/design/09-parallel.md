...
1. 单线程环境直接回退串行。
2. 若 `ParallelGuard` 检测到当前线程已处于并行执行上下文，则自动回退串行，避免库内部二次并行。
...
补充说明：`ParallelPool` 内部调用同样必须经过 `ParallelGuard`。若用户在自定义 pool 中再次调用并行 API，guard 会检测到二次并行并自动回退串行，与全局线程池行为一致；同时不允许嵌套 `ParallelPool` 实例，以避免引入额外调度语义。
