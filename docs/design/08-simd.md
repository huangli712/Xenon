...
单文件职责划分：`mod.rs` 负责 pulp 集成、Arch 缓存与 trait 定义，`vector.rs` 封装所有基于 `pulp::WithSimd` 的纯向量化逻辑。
...
pub fn dispatch_vector_binary_op<A>(op: BinaryOp, lhs: &[A], rhs: &[A], dst: &mut [A])
where
...
当前版本 SIMD 只覆盖下表列出的子集；未列出的操作统一由各语义模块的串行实现承担。是否实际进入 SIMD 还取决于元素类型、ISA 能力、统一对齐快路径与语义约束。当前版本在 `matrix` 相关范围内仅承载 **vector dot**，不展开矩阵乘法或其他 matrix 范围能力。以下表格给出当前设计的覆盖矩阵：

> **覆盖范围说明：** 当前版本 SIMD 路径按运算和类型逐步启用。对于尚未实现 SIMD kernel 的运算，由对应语义模块使用其串行实现，不影响公开 API 语义。

| 类别 | 操作 | 类型 | 当前设计目标 |
| ---- | ---- | ---- | ------------ |
| 二元算术 | `add` / `sub` / `mul` / `div` | `i32` / `i64` / `f32` / `f64` | 提供 SIMD path |
| 一元 | `abs` / `neg` | `i32` / `i64` / `f32` / `f64` | 提供 SIMD path；未满足约束时由语义模块串行实现处理 |
| 一元/数学函数 | `signum` / `square` / `sin` / `sqrt` / `exp` / `ln` / `floor` / `ceil` | 支持类型见需求说明书 §12 | 当前版本以算术运算为优先；`sin` / `sqrt` / `exp` / `ln` / `floor` / `ceil` 作为后续增强目标，`signum` / `square` 因无 ISA 原生支持走语义模块串行路径 |
| 比较 | `eq` / `ne` / `lt` / `gt` | `i32` / `i64` / `f32` / `f64` | 提供 SIMD path |
| 逻辑 | `not` | `bool` | 若可用 mask/byte-lane 向量化则走 SIMD，否则由语义模块串行实现 |
| 复数 | `conjugate` | `Complex<f32>` / `Complex<f64>` | 提供 SIMD path |
| 归约 | `sum` | `i32` / `i64` / `f32` / `f64` / `Complex<f32>` / `Complex<f64>` | 提供 SIMD path；整数无法保证 checked 语义时由语义模块串行实现 |
| 内积 | `dot` | `i32` / `i64` / `f32` / `f64` / `Complex<f32>` / `Complex<f64>` | 提供 SIMD path；整数无法保证 checked 语义时由语义模块串行实现 |
...
| `math::not` (`bool`) | serial in semantic module（仅在存在独立 mask/byte-lane kernel 时启用 SIMD） |
| `math::signum` / `square` / `sin` / `sqrt` / `exp` / `ln` / `floor` / `ceil` | serial in semantic module |
