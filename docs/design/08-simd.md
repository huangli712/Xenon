...
| 容差边界          | 整数精确一致；浮点/复数 `sum` 的有限值容差以 `13-reduction.md §6.3` 的数值容差表为权威基线；`dot` 的容差由 `12-matrix.md` 定义            |

**非有限值规则**：`sum` 的 `NaN` / `±Inf` / `±0.0` 规则与 `13-reduction.md §6.3` 保持一致——SIMD 后端不得用有限值容差掩盖 NaN、无穷值类别或零符号差异。
...
| 属性     | 值                                                                                                                                                                                                            |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 决策     | 整数 `sum` / `dot` 只有在可证明保持 checked arithmetic 语义时才使用 SIMD，浮点/复数 sum / dot 允许文档化容差。`sum` 的具体有限值容差引用 `13-reduction.md` 的容差表；`dot` 容差由 `12-matrix.md` 定义。       |
| 理由     | 需求说明书 §9、§28 明确要求公开能力可由 SIMD 加速实现，但不能破坏公开语义；整数路径只在可证明保持 checked arithmetic 语义时使用 SIMD，否则直接回退标量；浮点/复数路径则采用受控的累加顺序变化与显式容差记录。 |
