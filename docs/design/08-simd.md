...
| 范围内   | 当前版本 SIMD 仅覆盖 `f32`/`f64`/`i32`/`i64` 的逐元素二元算术（`add`/`sub`/`mul`/`div`），以及 `f32`/`f64`/`i32`/`i64`/`Complex<f32>`/`Complex<f64>` 的 `sum` / `dot` |
| 范围外   | 数学函数 SIMD（`sin`/`sqrt`/`exp`/`ln`/`floor`/`ceil`）、矩阵乘法、非 `sum` 归约，以及一元/比较/复数逐元素专用 SIMD kernel |
| 标量回退 | 一元、数学函数、复数逐元素与比较相关操作保留公开 API，但当前版本统一走 scalar path only |
...
pub trait SimdKernel<A: Copy + Send + Sync + 'static>: Send + Sync {
    ...
    // NOTE: These unary hooks are kept for future SIMD extension.
    // In the current version, `neg`/`abs` do not have dedicated SIMD kernels
    // and therefore execute through the scalar fallback path.
...
当前版本 SIMD 仅覆盖 `f32`/`f64`/`i32`/`i64` 的逐元素二元算术，以及 `f32`/`f64`/`i32`/`i64`/`Complex<f32>`/`Complex<f64>` 的 `sum()` 与 `dot()` kernel；其余运算统一回退标量实现。该覆盖范围与 `11-math.md` 的 SIMD 说明保持一致。以下需求说明书 §12 / §13 中的操作在当前版本**不提供专门的 SIMD kernel**，即使启用了 `simd` feature，也统一回退到标量路径：
