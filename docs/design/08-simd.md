...
SIMD 后端模块是 Xenon 张量库的可选性能加速层，通过 `pulp` crate 提供跨平台 SIMD 抽象。根据需求说明书 §9.1，SIMD 设计目标覆盖 §12 中定义的全部逐元素运算，以及 `sum` / `dot`；其中不同操作可按类型、ISA 与语义约束选择专用 SIMD kernel 或自动回退标量。该模块默认关闭，通过 `features = ["simd"]` 启用。
...
| 逐元素运算 | 以 §12 全部逐元素运算为目标覆盖范围：算术、一元、数学函数、比较、复数 `conjugate`，以及可向量化时的 `bool not` | 复杂线性代数 (矩阵分解) |
...
| 类型     | 内容                                                                                 |
| -------- | ------------------------------------------------------------------------------------ |
| 需求映射 | `Requirements §9.1`, `§9.3`, `§27`, `§28.3`, `§28.5`                                  |
| 范围内   | SIMD 设计目标覆盖 §12 中定义的全部逐元素运算：算术（`add`/`sub`/`mul`/`div`）、一元（`abs`/`signum`/`neg`/`square`）、数学函数（`sin`/`sqrt`/`exp`/`ln`/`floor`/`ceil`）、比较（`eq`/`ne`/`lt`/`gt`）、复数 `conjugate`、可向量化时的 `bool not`，以及 `sum` / `dot` |
| 范围外   | 矩阵乘法、非 `sum` 归约、复数 `norm` 的专用 SIMD kernel，以及 SVE 当前版本实现 |
...
>
> **复数 SIMD 策略：** `Complex<f32>` 和 `Complex<f64>` 在当前设计中优先覆盖 `sum()` / `dot()` 与 `conjugate()`；`norm()` 因输出类型变化与额外平方和/开方语义，当前版本保持标量回退。实现上采用 AoS 输入 + 寄存器内重排，按实部/虚部分离后执行
...
pub trait SimdKernel<A: Copy + Send + Sync + 'static>: Send + Sync {
    ...
    /// use a different accumulation order, but per `require.md` §9.3 / §28.3 the
    /// documented result must stay within at most 2 ULP of the scalar reference
    /// for `f32`/`f64`, and the same 2 ULP bound applies component-wise to complex values.
    ...
    // NOTE: `fill`/`fill_zero` remain in the trait to keep the execution surface
    // stable for future SIMD kernels. The current version handles them via the
    // scalar fallback implementation only.
...
当前版本 SIMD 的设计目标是覆盖 `11-math.md` / 需求说明书 §12 中定义的全部逐元素运算，以及 `sum()` 与 `dot()`；是否实际进入 SIMD 取决于元素类型、ISA 能力、对齐条件与语义约束。以下表格给出当前设计的覆盖矩阵：

| 类别 | 操作 | 类型 | 当前设计目标 |
| ---- | ---- | ---- | ------------ |
| 二元算术 | `add` / `sub` / `mul` / `div` | `i32` / `i64` / `f32` / `f64` | 提供 SIMD path |
| 一元 | `abs` / `signum` / `neg` / `square` | `i32` / `i64` / `f32` / `f64` | 提供 SIMD path，不满足约束时回退标量 |
| 数学函数 | `sin` / `sqrt` / `exp` / `ln` / `floor` / `ceil` | `f32` / `f64` | 目标提供 SIMD path，ISA 不支持或误差无法受控时回退标量 |
...
| 复数 | `norm` | `Complex<f32>` / `Complex<f64>` | 当前版本标量回退 |
...
│ • ISA-specific preferred alignment met?                         │
│   SSE/NEON: 16B, AVX2: 32B, AVX-512: 64B                        │
│   (unaligned load/store may still be used when kernel supports) │
...
/// 4. Preferred alignment matches the active ISA (SSE/NEON 16B,
///    AVX2 32B, AVX-512 64B); kernels that support unaligned
///    load/store may still use SIMD below this threshold
///
    ...
    fn with_simd<S: Simd>(self, simd: S) -> Self::Output {
        ...
        for i in 0..chunks {
            ...
            unsafe {
                ...
                // Preferred alignment has been checked for the active ISA;
                // kernels with unaligned support may also dispatch here.
...
> **设计决策：** 对于逐元素运算，SIMD 与标量路径必须保持相同公开语义；对 `add` / `sub` / `mul` / `div`、比较、`conjugate`、以及可向量化的一元/逻辑操作，若对应 ISA kernel 已实现，则结果应与标量逐元素求值一致。对于归约和内积，当前版本必须实现 SIMD 设计路径，并遵循 `require.md` §9.1 / §9.3 / §28.3 的数值约束：整数结果必须与标量逐步 checked arithmetic 精确一致；`f32` / `f64` / `Complex<f32>` / `Complex<f64>` 允许因结合顺序变化产生有界差异，但最终结果与标量参考值的偏差最多为 2 ULP（复数按实部、虚部分量分别计算）。
>
> **一致性说明：** 对于逐元素操作（add、mul 等），SIMD 和标量路径产生逐位一致的结果。
> 对于归约/内积操作，Xenon 不接受未记录的“近似一致”；若某个 SIMD 内核无法满足文档定义的数值语义与容差边界，则不走 SIMD 路径。
>
> **覆盖说明：** 当前版本的设计目标是覆盖 §12 全部逐元素运算以及 `sum` / `dot`；其中数学函数、`norm`、`bool not` 等是否实际进入 SIMD，取决于具体 ISA kernel 是否存在且能满足本文语义约束。未覆盖或不满足约束的情形统一自动回退标量路径。这一点与 `11-math.md` 的模块边界说明保持一致。
...
| `f32` | lane 内 pairwise/Kahan-style 累加，水平合并允许不同顺序 | `mul` 后进入同一累加流程 | 与标量参考值偏差最多 2 ULP |
| `f64` | lane 内 pairwise/Kahan-style 累加，水平合并允许不同顺序 | `mul` 后进入同一累加流程 | 与标量参考值偏差最多 2 ULP |
| `Complex<f32>` | 将 AoS 数据重排为实/虚 lane，分别累加后重组 | 先执行共轭乘法：`conj(lhs_i) * rhs_i`，再分别累加实部与虚部 | 每个分量相对标量参考值偏差最多 2 ULP |
| `Complex<f64>` | 将 AoS 数据重排为实/虚 lane，分别累加后重组 | 先执行共轭乘法：`conj(lhs_i) * rhs_i`，再分别累加实部与虚部 | 每个分量相对标量参考值偏差最多 2 ULP |
...
- [ ] **T4b**: 实现整数 `sum` 与 widening `dot` SIMD 路径
  - 文件: `src/simd/vector.rs`
  - 内容: 为 `i32`/`i64` 实现 widening accumulation、scalar merge 与 overflow validation
...
- [ ] **T6b**: 编写 reduction / dot 语义与容差测试
  - 文件: `src/simd/scalar.rs` (#[cfg(test)])
  - 内容: 覆盖浮点、复数、整数 widening 的语义约束与容差边界
...
| 类型                 | 说明                                                                                 |
| -------------------- | ------------------------------------------------------------------------------------ |
| Recoverable error    | 无专属 recoverable error；SIMD 不可用、类型不支持、对齐/未对齐访问策略不满足或当前 ISA 无法满足语义约束时均自动回退标量路径 |
...
| 容差边界             | integer: exact; float/complex reduction and dot may differ from scalar accumulation order within at most 2 ULP per real component, per `require.md` §9.3 / §28.3 |
...
| 属性     | 值                                                                                                                                             |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| 决策     | SIMD 设计目标覆盖 §12 全部逐元素运算以及 `sum` / `dot`；整数 `sum` / `dot` 只有在可证明保持 checked arithmetic 语义时才使用 SIMD，浮点/复数 reduction / dot 允许文档化容差                                     |
| 理由     | `require.md` §9.1 / §9.3 / §28.3 明确要求公开能力可由 SIMD 加速实现，但不能破坏公开语义；因此整数路径拒绝仅靠 widening accumulation 的方案，浮点/复数路径则采用受控的累加顺序变化与显式容差记录。 |
...
| 内存对齐   | 首选按 ISA 对齐：SSE/NEON 16B、AVX2 32B、AVX-512 64B；支持未对齐访问的 kernel 可直接使用 unaligned load/store，否则回退标量路径   |
| 尾部处理   | 标量循环处理尾部，简单安全                   |
