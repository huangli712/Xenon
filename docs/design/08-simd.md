...
SIMD 后端模块是 Xenon 张量库的可选性能加速层，通过 `pulp` crate 提供跨平台 SIMD 抽象。该模块默认关闭，通过 `features = ["simd"]` 启用。
...
| 逐元素运算 | `SimdKernel` 直接覆盖二元算术与 `neg`  |
| 归约运算   | `sum` 的 SIMD 加速与向量化可用性约束   |
| 内积运算   | `dot` 的 SIMD 加速与向量化可用性约束   |
...
| 原则         | 体现                                                                                                    |
| ------------ | ------------------------------------------------------------------------------------------------------- |
| 结果一致性   | 整数 `sum` / `dot` 必须等价于逐步 checked arithmetic，浮点/复数 `sum` / `dot` 允许在容差内偏离标量结果  |
| 双层加速     | SIMD 与并行可同时启用                                                                                   |
...
| 范围内   | 本文覆盖矩阵中列出的 SIMD 子集：`add/sub/mul/div`、`neg`、`sum/dot`，以及复数逐元素算术   |
...
`SimdKernel` 的二元运算入口 `dispatch_vector_binary_op(...) -> bool` 通过返回值显式表达"是否实际进入 SIMD"——`false` 时调用方负责走标量回退，`dst` 未被修改。`SimdKernel::sum(&self, data: &[A]) -> A` 与 `SimdKernel::dot(&self, lhs: &[A], rhs: &[A]) -> A` 因为返回归约结果 `A` 本身，签名上无法承载 admission 信号。本节明确这两个入口的契约：

**契约 1：SIMD sum / dot 入口的调用前提（调用方义务）**

调用 `SimdKernel::sum` / `SimdKernel::dot` 之前，调用方模块（`13-reduction.md` / `12-matrix.md`）必须自行完成 type gate，确保被调用类型在本平台/本 ISA 上属于 §5.6 表中"已实现"或"已验证 widening 可用"的子集。具体规则：

| 元素类型 | sum | dot | 调用方 gate 决策 |
|---|---|---|---|
| `f32` / `f64` | 已实现 | 已实现 | 可直接调用 SIMD 入口 |
| `Complex<f32>` / `Complex<f64>` | 已实现 | 已实现 | 可直接调用 SIMD 入口 |
| `i32` / `i64`（无已验证 widening） | 条件实现 | 条件实现 | 不得调用 SIMD 入口；调用方在 dispatch 之前自行选 Serial |
...
- 对"不得调用"的类型，实现侧不提供 SIMD 实现入口（在类型系统层面以 trait bound + 编译期分派排除，而非运行时分支）；调用方若违反契约 1 在编译期即报错，避免运行时无信号失败
...
| 非对齐数据                | 由 `simd/` 后端自行决定走 unaligned kernel 或回退标量；dispatch 层不再以非对齐为由拒绝 SIMD 路径 |
...
| 容差边界          | 整数精确一致；浮点/复数 `sum` 的有限值容差以 `13-reduction.md §6.3` 的数值容差表为权威基线；`dot` 的容差由 dot/matrix 语义文档定义，未定义前**不得**启用会改变合并顺序的 SIMD `dot` 路径；`00-coding.md §8.4` 仅作为实现/测试参考；`需求说明书 §28.3` 为顶层需求来源。 |

**非有限值规则**：`sum` 的 `NaN` / `±Inf` / `±0.0` 规则与 `13-reduction.md §6.3` 保持一致——SIMD 后端**不得**用有限值容差掩盖 NaN、无穷值类别或零符号差异。`dot` 在自身容差表落地之前同样适用此规则。
...
| 属性     | 值                                                                                                                                                                                                               |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 决策     | SIMD 只覆盖本文列出的子集以及 `sum` / `dot`；整数 `sum` / `dot` 只有在可证明保持 checked arithmetic 语义时才使用 SIMD，浮点/复数 reduction / dot 允许文档化容差。`sum` 的具体有限值容差**不**在本 SIMD 文档内重复定义，统一引用 `13-reduction.md §6.3` 的容差表；`dot` 容差由 dot/matrix 语义文档独立定义。 |
| 理由     | 需求说明书 §9、§28 明确要求公开能力可由 SIMD 加速实现，但不能破坏公开语义；因此整数路径只在可证明保持 checked arithmetic 语义时使用 SIMD，否则直接回退标量；浮点/复数路径则采用受控的累加顺序变化与显式容差记录。容差表中心化在归约语义文档中可避免容差定义在多处漂移。 |
...
| 替代方案 | 保留"并行 worker 内强制标量"策略 — 放弃，明确放弃可观吞吐；同时 SIMD 内部容差规则与 chunk 间合并顺序由 `parallel` 模块独立控制，互斥并不能换来更强的数值确定性                                                                          |
...
| 理由     | 签名为 `-> ()`，无法显式表达"未进入 SIMD"，与 §1.1 "`simd/` 永不承担标量回退" 形成无法闭合的控制流。明确的布尔信号让调用侧（`math` / `matrix::dot` / `reduction`）可以确定是否需要执行自己的串行实现，回退归属彻底单向化 |
| 替代方案 | `simd/` 内部承担标量回退 — 放弃，违反 §1.1 职责边界                                                                                                  |
