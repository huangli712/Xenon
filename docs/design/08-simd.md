...
│   See 11-math.md §4, 13-reduction.md §4, 12-matrix.md §4        │
└─────────────────────────┬───────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────────┐
...
当前版本 SIMD 仅覆盖逐元素算术/一元运算和整数归约；数学函数（如 sin/cos/exp/log）和浮点归约/内积均回退标量实现。以下需求说明书 §12 / §13 中的操作在当前版本**不提供专门的 SIMD kernel**，即使启用了 `simd` feature，也统一回退到标量路径：
