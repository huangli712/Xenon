...
| 逐元素运算 | 加减乘除、abs、neg 等已覆盖操作的向量化 | `sqrt` 等当前未覆盖操作、复杂线性代数 (矩阵分解) |
...
├── crate::tensor             # 通过 TensorBase 暴露连续性/布局查询
...
### 4.4a 当前版本的 SIMD 覆盖范围

以下需求说明书 §12 / §13 中的操作在当前版本**不提供专门的 SIMD kernel**，即使启用了 `simd` feature，也统一回退到标量路径：
...
│ 保证：                                                          │
│ • 逐元素操作：SIMD 与标量逐位一致                                │
│ • 归约操作：仅在结果与标量路径一致时启用 SIMD，否则回退标量       │
│ • 内积操作：仅在结果与标量路径一致时启用 SIMD，否则回退标量       │
└─────────────────────────────────────────────────────────────────┘
```
...
>
> **覆盖说明：** 当前版本要求 SIMD 后端承接逐元素运算、归约和内积这三类能力，但并不要求 §12 中所有逐元素操作都已有独立向量化实现；未覆盖的操作维持公开 API 不变并自动回退标量路径。
...
| 属性     | 值                                                                                                                                             |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| 决策     | 逐元素运算须逐位一致；归约/内积仅在可证明与标量路径一致时启用，否则回退标量                                                                    |
| 理由     | 逐元素运算的 SIMD 实现与标量在数学上等价，应完全一致。归约和内积只有在其 SIMD 实现能证明与标量路径保持一致时才可启用，否则必须回退到标量路径。 |
...
### 10.2 性能数据（目标值，非当前默认路径）

> 说明：下表中的 `sum` / `dot` SIMD 数据仅表示“在未来具备一致性证明的内核落地后”的目标值；按照 §9 ADR-2，当前默认实现中浮点 `sum` / `dot` 仍保守回退到标量路径。
