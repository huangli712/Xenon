...
SIMD 后端模块是 Xenon 张量库的可选性能加速层，通过 `pulp` crate 提供跨平台 SIMD 抽象。根据需求说明书 §9.1，当前版本优先覆盖算术、比较、`conjugate`、`sum` 与 `dot` 等已实现子集；数学函数与 `norm` 在当前版本仍保持标量路径，并作为后续增强目标。该模块默认关闭，通过 `features = ["simd"]` 启用。
...
| 一元/数学函数 | `signum` / `square` / `sin` / `sqrt` / `exp` / `ln` / `floor` / `ceil` | 支持类型见需求说明书 §12 | 当前版本保持标量路径，后续优先级低于已列出的算术/比较/归约/内积 SIMD 覆盖 |
...
>
> **覆盖说明：** 当前版本只覆盖本文表格中列出的 SIMD 子集；其中 `bool not` 是否实际进入 SIMD，取决于具体 ISA kernel 是否存在且能满足本文语义约束。数学函数与 `norm` 当前仍走标量路径，作为后续增强目标逐步补齐；这不改变 `math` 模块对这些逐元素运算的公开 API 承诺。这一点与 `11-math.md` 的模块边界说明保持一致。
