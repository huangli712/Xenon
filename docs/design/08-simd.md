...
| 归约运算   | `sum` 的 SIMD 加速与向量化可用性约束                                 | `max`/`argmax` 等其他归约                  |
| 内积运算   | `dot` 的 SIMD 加速与向量化可用性约束                                 | 外积、矩阵乘法                             |
...
| 范围内   | 本文覆盖矩阵中列出的 SIMD 子集：`add`/`sub`/`mul`/`div`、`abs`/`neg`、`eq`/`ne`/`lt`/`gt`、复数 `conjugate`、可选 `bool not`、以及 `sum` / `dot`；未列出能力由各语义模块的串行实现承担 |
...
| 阶段 | 类别 | 操作 | 类型 | 状态说明 |
| ---- | ---- | ---- | ---- | -------- |
| 当前覆盖 | 二元算术 | `add` / `sub` / `mul` / `div` | `i32` / `i64` / `f32` / `f64` | 提供 SIMD path |
| 当前覆盖 | 一元 | `abs` / `neg` | `i32` / `i64` / `f32` / `f64` | 提供 SIMD path |
| 当前覆盖 | 比较 | `eq` / `ne` / `lt` / `gt` | `i32` / `i64` / `f32` / `f64` | 提供 SIMD path |
| 当前覆盖 | 复数 | `conjugate` | `Complex<f32>` / `Complex<f64>` | 提供 SIMD path |
| 当前覆盖 | 归约 | `sum` | `i32` / `i64` / `f32` / `f64` / `Complex<f32>` / `Complex<f64>` | 提供 SIMD path；整数无法保证 checked 语义时透明回退 |
| 当前覆盖 | 内积 | `dot` | `i32` / `i64` / `f32` / `f64` / `Complex<f32>` / `Complex<f64>` | 提供 SIMD path；整数无法保证 checked 语义时透明回退 |
| 规划覆盖 | 一元/数学函数 | `signum` / `square` | 支持类型见需求说明书 §12 | 当前无独立 SIMD kernel，先走语义模块串行路径 |
| 规划覆盖 | 一元/数学函数 | `sin` / `sqrt` / `exp` / `ln` / `floor` / `ceil` | 支持类型见需求说明书 §12 | 作为后续增强目标逐步补齐 |
| 条件覆盖 | 逻辑 | `not` | `bool` | 仅在独立 mask/byte-lane kernel 就绪时启用，否则透明回退 |

| 操作类别 | 具体运算 | SIMD 覆盖状态 | 说明 |
| -------- | -------- | ------------- | ---- |
| 算术运算 | add/sub/mul/div (f32/f64) | ✅ 已覆盖 | |
| 算术运算 | add/sub/mul/div (i32/i64) | ⚠️ 仅可证明等价时启用 | 整数 checked 语义约束 |
| 算术运算 | add/sub/mul/div (Complex) | 📋 当前版本串行回退 | |
| 数学函数 | sin/sqrt/exp/ln/floor/ceil | 📋 当前版本串行回退 | |
| 一元运算 | neg/square (f32/f64) | ✅ 已覆盖 | |
| 一元运算 | abs/signum | 📋 当前版本串行回退 | |
| 复数运算 | conjugate | ✅ 已覆盖 | |
| 归约 | sum (f32/f64) | ✅ 已覆盖 | |
| 归约 | sum (i32/i64) | ⚠️ 仅可证明等价时启用 | |
| 内积 | dot (f32/f64) | ✅ 已覆盖 | |
| 内积 | dot (Complex) | 📋 当前版本串行回退 | |
| 逻辑 | not (bool) | 📋 当前版本串行回退 | |

| Public API | 当前 SIMD 状态 | 阶段 |
| ---------- | ---------------- | ---- |
| `math::add` / `sub` / `mul` / `div` | implemented | 当前覆盖 |
| `math::abs` / `neg` | implemented | 当前覆盖 |
| `math::eq` / `ne` / `lt` / `gt` | implemented | 当前覆盖 |
| `math::conjugate` | implemented | 当前覆盖 |
| `reduction::sum` | implemented | 当前覆盖 |
| `matrix::dot` / `dot` | implemented | 当前覆盖 |
| `math::signum` / `square` | serial fallback in semantic module | 规划覆盖 |
| `math::sin` / `sqrt` / `exp` / `ln` / `floor` / `ceil` | serial fallback in semantic module | 规划覆盖 |
| `math::not` (`bool`) | serial fallback unless dedicated mask kernel exists | 条件覆盖 |

> **`SimdKernel::dot()` 说明：** `dot()` 为当前版本正式能力。`f32` / `f64` / `Complex<f32>` / `Complex<f64>` 可以使用 SIMD dot kernel；`i32` / `i64` 仅在可证明与标量逐步 checked arithmetic 等价时使用 SIMD，否则由对应语义模块转回串行实现。
...
SIMD 条件判断已迁移至内部 `dispatch.rs` 模块（`can_use_simd`）。`simd/` 模块只提供纯向量化执行能力。上层仍按统一规则检查 feature、连续性、对齐、元素类型与运行时可用 lane 宽度后，才进入 `simd/`。

### 5.7 Good/Bad 对比示例

```rust,ignore
// Good - semantic module dispatches first, SIMD backend stays pure-vectorized
...
fn add_arrays<A: SimdElement>(
    ...
    if dispatch::can_use_simd(lhs, rhs, dst, is_contiguous) {
        simd::dispatch_vector_binary_op(BinaryOp::Add, lhs, rhs, dst);
    } else {
        serial::add(lhs, rhs, dst);
    }
}

// Bad - calling the SIMD backend directly without semantic dispatch
fn add_arrays_bad<A: SimdElement>(lhs: &[A], rhs: &[A], dst: &mut [A]) {
    // FORBIDDEN: bypasses dispatch.rs path selection
    simd::dispatch_vector_binary_op(BinaryOp::Add, lhs, rhs, dst);
}
```
...
│   Check order: AVX-512 -> AVX2+FMA -> SSE4.1 -> NEON            │
└─────────────────────────┬───────────────────────────────────────┘
                          │
           ┌──────────────┼──────────────┐
           │              │              │
...
| 消费（输入） | `math` / `reduction` / `dot` | `dispatch.rs` / `dispatch_vector_binary_op()` / `SimdKernel::sum()` / `SimdKernel::dot()` | 上层语义模块先完成形状和类型裁决，再在 `dispatch.rs` 中做路径选择，只有 SIMD 分支才传入兼容切片 |
