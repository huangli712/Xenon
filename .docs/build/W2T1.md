...
- W1T3 完成：`src/lib.rs` 存在且包含 `pub mod prelude;` 声明
- W1T3 模块演进协议表规定：`pub mod error;` 由本 task (W2T1) 负责写入 lib.rs

## 实施步骤

### Step 1: 在 lib.rs 中声明 error 模块

按 W1T3「lib.rs 模块声明演进协议」，本 task 负责在 `src/lib.rs` 中追加 `pub mod error;` 声明。
在 `pub mod prelude;` 之前（或之后，按 `01-architecture.md §8` 第 601 行位置）插入：

```rust
pub mod error;
```

验证：`cargo check` 需要 `src/error.rs` 存在（Step 2 创建）。
...
### Step 5: 定义 Conversion、Argument、Layout、Storage 辅助类型
