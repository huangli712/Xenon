...
| 引入 `RealScalarBits` crate-private 扩展 trait（§5.2 L358-359 原设计要求 `impl RealScalar`御不暴露 `to_bits()`） | 03-element §5.3 `RealScalar` 不暴露 to_bits/from_bits；在 W29T1 中作为本地混入 限制仅 f32/f64、不外露到 prelude |
