...
- **W4T1 已完成**：`crate::element::Element` trait 可用作类型约束。
...
| `*const A` → `*const c_void` 使用 `pointer::cast()` 而非 `as` 强转 | 保留指针 provenance，未来兼容 strict-provenance lint；同时与设计 §5.4 注释中 "erased pointer" 叙述一致。**这是对设计文档 23-ffi.md §5.3.1 line 320 / 334 原文 `as *const core::ffi::c_void` 强转的主动改进**（设计不变，仅实现选择 cast 完成同一语义 + 未来兼容性）；任何处改回 `as` 强转都仍然正确，但以上改进报告 `lints.clippy.ptr_as_ptr` 提升为 deny 后会变为错误。 |
...
- `From` impl 使用 `e.data.cast()` 而非设计文档中的 `as *const c_void`：二者语义等价，后者是 Rust 2024 strict-provenance 下的推荐语法。如果 clippy `ptr_as_ptr` lint 未来提升为 deny，原变 `as` 形式依然可编译通过。
- 本 task 单元测试不依赖任何 W22 构造 API：`test_tensor_export_to_raw_conversion` 使用 `NonNull::<f64>::dangling()` 处理空张量路径；`test_tensor_export_mut_to_raw_conversion` 使用本地 `[1.0_f64, 2.0, 3.0]` Vec 处理非 dangling 路径。两个路径不需要 W22。
