...
> **导出**：`compute_layout_flags` 与 `flags_for_f_layout` 仅在 `layout` 模块内通过 `pub(crate)` 暴露；不在 `pub use` 列表中，外部不可直接访问。Tensor 层（W8）通过 `crate::layout::compute_layout_flags::<A, D>(...)` 调用。`pub(crate) fn` 已提供跨模块可见性，无需额外 `pub(crate) use` 导出。
