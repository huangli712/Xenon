...
| 属性 | 行为 |
|------|------|
| 操作 | `cat(axis, &[TensorView])` 沿指定轴拼接多个数组，返回新 Owned 数组（需拷贝） |
...
| 属性 | 行为 |
|------|------|
| 操作 | `stack(axis, &[TensorView])` 沿新轴堆叠多个数组，返回新 Owned 数组（需拷贝） |
...
| 属性 | 行为 |
|------|------|
| 签名 | `fn flatten_view(&self) -> Result<TensorView<A, Ix1>, LayoutMismatch>` |
...
| 输出布局 | `map`/`mapv` 返回 F-contiguous 布局的 Owned 数组（与项目默认 F-order 一致） |
| 空数组 | 返回同形状空数组，`f` 不被调用 |
