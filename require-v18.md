...
| MSRV 策略 | 遵循 N-2 政策（支持当前 stable 及前两个版本）；MSRV 提升视为 minor 版本变更（非 breaking），须在 CHANGELOG 中明确标注 |
| 单态化控制 | 建议通过 feature gate 控制实例化范围（如 `int-types`、`float-types` feature），减少不必要的泛型展开 |
| 编译时间目标 | 全 feature 干净编译 ≤ 120 秒（不含依赖编译）；增量编译 ≤ 15 秒（单文件修改） |
| 二进制体积 | 仅启用 `std` feature 时，单类型（如 f64）Release build 二进制增量 ≤ 500 KB |
| API 弃用策略 | 废弃 API 须标注 `#[deprecated]` 并在文档中说明替代方案；deprecated API 至少保留 2 个 minor 版本（约 6 个月）后才可移除；major 版本可移除已 deprecated 超 2 个 minor 版本的 API |
...
| sealed 实现者 | 仅库内预定义类型：i8/i16/i32/i64/i128/u8/u16/u32/u64/u128/f32/f64/bool/Complex\<f32\>/Complex\<f64\> |
...
| HAS_ZERO_STRIDE | 零步长（缓存） | 存在步长为 0 的维度。两种来源：(1) 广播产生的 size-1 维度（逻辑上重复该元素）；(2) unsqueeze 插入的 size-1 轴（§11.8，步长设为 0 以保持连续性）。优化路径据此判断不可按简单步长递增遍历，但连续性标志不受影响 |
...
| 元素数 ≥ 并行阈值 且 parallel 启用 且 非连续内存（步长 ≠ 1） | 标量 + 并行路径（每线程使用标量迭代，按块分割非连续数据） |
...
> **约束**：`eq`/`ne` 仅要求 `PartialEq`；`lt`/`le`/`gt`/`ge` 要求 `PartialOrd`（浮点类型因 NaN 为偏序）。所有比较方法仅通过方法调用提供，不提供 `< <= > >=` 运算符语法（见 §13.2 设计决策）。支持广播。返回的 bool Tensor 可用于 `where()`、`mask()`、`compress()` 等条件操作。
...
| 操作 | 说明 | 输入类型约束 | 返回类型 |
|------|------|--------------|----------|
| unique | 返回唯一值（排序后） | Element + Ord（浮点类型使用 total_cmp 提供全序） | Tensor1<A> |
...
| 属性 | 行为 |
|------|------|
| 操作 | `reshape(shape) -> Result<TensorView<A, D'>, ShapeError>` 改变数组形状，元素总数须不变 |
| 零拷贝条件 | 输入须为连续内存（F-contiguous 或 C-contiguous）；连续输入仅修改 shape/strides 元数据，不拷贝数据 |
| 非连续输入 | 返回 `LayoutMismatch` 错误；调用方须先 `.to_contiguous()` 再 reshape，或使用 `.reshape_owned()` |
| reshape_owned | `reshape_owned(shape) -> Tensor<A, D>` 始终成功，非连续时先拷贝为 F-contiguous 再 reshape；连续时等价于 reshape + to_owned |
| 输出布局 | 零拷贝路径：继承输入的连续性方向（F-contiguous → F-order strides；C-contiguous → C-order strides） |
...
| 零拷贝 | 始终零拷贝 |
| 与 transpose 关系 | `permute` 为 `transpose(axes)` 的别名，提供 NumPy 兼容命名 |
...
| step = 0 | 静态维度：编译错误（宏展开时检测字面量 0）；动态维度：运行时 panic |
...
| unsafe empty_uninit | 返回 `Tensor<MaybeUninit<A>, D>`，未初始化内存（见下方语义） |
...
| 整数溢出 | 整数 arange 的累加使用 `checked_add`，溢出时在该点停止（不 panic） |
| 浮点精度 | 浮点 step 的累积误差可能导致最后一个元素的值略偏离预期 end；与 NumPy `np.arange` 行为一致 |
| 返回类型 | 始终 `Tensor1<A>`，布局为 F-contiguous（1D 即 C-contiguous） |
...
| num = 0 | 返回空 Tensor1 |
| num = 1 | 返回仅含 `start` 的 Tensor1 |
| 精度 | 使用 `(end - start) / (num - 1)` 计算步长，浮点累积误差不可避免 |
...
- 比较运算（仅方法调用 `.lt()`/`.le()`/`.gt()`/`.ge()`，不提供运算符语法，见下方）
...
| map/mapv/mapv_inplace | 映射操作（语义见下方） |
| cast | 显式类型转换 |
...
| to_contiguous() | 返回连续布局的新 Tensor：非连续输入输出 F-contiguous；F-contiguous 输入等价于 to_owned()；C-contiguous 输入等价于 to_owned() |
...
| `From<Vec<A>>` | `Tensor1<A>` | 见上方 From 转换表 |
| `From<[A; N]>` | `Tensor1<A>` | 见上方 From 转换表 |
...
| **复合错误** | ShapeError | reshape 系列操作的聚合错误（§11.2），包含 InvalidShape 和 LayoutMismatch 两种情况 | Result |
...
> `ShapeError` 是 reshape 系列 API（§11.2）的专用错误类型，将 reshape 可能遇到的两种失败（形状不匹配 / 布局不满足）聚合为一个枚举，简化调用方错误处理。其他 API 使用对应场景的独立错误类型（如 `LayoutMismatch` 用于 `flatten_view`、`InvalidShape` 用于 `cat`）。

**API → 错误类型映射**：

| API 类别 | 返回的错误类型 | 说明 |
|----------|---------------|------|
| reshape / reshape_owned | `ShapeError` | 聚合错误（InvalidShape ∪ LayoutMismatch） |
...
| 精度度量 | 超越函数使用相对误差 `\|computed - expected\| / \|expected\|`；归约使用绝对误差 `\|computed - expected\|`；加减乘使用 ULP（Unit in the Last Place）验证（允许 ≤ 1 ULP） |
